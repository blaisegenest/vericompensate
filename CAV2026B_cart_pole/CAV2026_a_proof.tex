...
\begin{proof}
	(of the third claim.) Without loss of generality, we assume the Lipschitz constant for $succ$ and $\sigma$ (on $P$) is 1. 
	
	
	First, since $x\in R_{\delta/2}$ and $||y-x||<\delta/4$,  by assumption and definition, we have that $succ(y,\sigma(x))\subseteq  R_{\delta/2}$. 
	
	Now, because both $succ$ and $\sigma$ (on $P$) are Lipschitz, we have that \begin{align*}
		succ(y,\sigma(y)) &\subseteq succ(y,\sigma(B_{\delta/4}(x))) \\
		&\subseteq succ(y,B_{\delta/4}(\sigma(x))) \\
		&\subseteq B_{\delta/4}(succ(y,\sigma(x))) \\
		&\subseteq B_{\delta/4}(R_{\delta/2}).
	\end{align*}
