...
\begin{proof}
	In this proof, we use $R_i$ to denote each $R_{\delta/2}(I)\cap Q_i$. We need to show $R_{\delta/2}(I)$ is attractive in $\bigcup_{i\in I}B_\varepsilon(R_i)\cap Q_i$.
	
	By assumption and definition, each $Q_i$ is assigned with a Lipschitz continuous function $f_i$ with Lipschitz constant $K_i$. Let $K = \max_{i\in I} K_i$ and $\varepsilon = \frac{\delta}{2K+1}$. 
	
	For a point $y$ in $B_\varepsilon(R_i)\cap Q_i$, there exists $x\in R_i$ such that $d(x,y)<\varepsilon$. We consider $succ_p(y)$ and $succ_p(x)$. 
	
	Since $x,y\in Q_i$, $d(p(x),p(y))\leq K_i d(x,y) < K \varepsilon<\delta/2$. Because $x\in R_i\subseteq R_{\delta/2}(I)$, by definition, since $succ_p(y)\in B_{K_i\varepsilon}(succ_p(x))$ and $x\in R_{\delta/2}(I)$, then $succ_p(y)\in R_{\delta/2}(I)$.
