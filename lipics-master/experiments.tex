...
%\vspace{-0.2cm}
$$comp(b,b')=\sum_a \sum_{c,c' \in Z} \val_{\vx}(a) \min( weight(abcd),weight(ab'c'd))$$ and the associated $comp(b)=\sum_{b' \in Z} comp(b,b') + comp(b',b)$. We then select iteratively nodes $b$ (first layer) or $c$ (second layer) by selecting the heaviest $comp(b)$ or $comp(c)$. {\color{red} The intuition behind the formula is simple: $\val_{\vx}(a)weight(abcd)$ is the naive contribution from node $a$ to the target $d$. And the min of two paths is a natural number for the strength of compensating: if one of them is very small, the effect of compensating will also be very small.}
