...
\begin{tikzpicture}
	\begin{axis}[
		...
		axis lines=center,
		samples=100, 
	 unit vector ratio=1 1 1, scale=1.5, xtick   = {-2,2},
	 ...
	 yticklabels = {},
		...
		\addplot[blue, thick] {0}; 
			...
			\node[label={above:$(\gamma_i,\gamma_i)$}] at (axis cs: 1.45, 1.7) {};
	\end{axis}
\end{tikzpicture}

\vspace*{10ex}

The relation between $y_i$, $x_i$ and $\hat{y}_i$ is $$\hat{y}_i = \ReLU(x_i+y_i)-\ReLU(x_i).$$ And its plots is as follows:
