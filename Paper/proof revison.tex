...
\begin{lemma}
	...
	%	for minimal value,	
	%	$ \min(n)=\sum_{m \in L}W_{m n)\min(\)+\boldsymbol{b}_n $ and
		%	$ \min(\hat{n}_s)=ReLU(\sum_{m \in L}W_{m n)\min(\hat{m}s}+\boldsymbol{b}_n)$
	\end{lemma}
	...
	%	\begin{itemize}
		...
		%	as \boldsymbol{b}. In the simplified $D'$, there is no compensating path because of the definition of $I$. Therefore we can apply Theorem \ref{th1} (make a more precise corollary) to get that $z(\cdot,\boldsymbol{x}^0_J)$ reaches its maximal value for $\boldsymbol{x}_I=\boldsymbol{x}_I^*$, and we are done.
		%	
		%	The second statement is simpler because for each fixed partial input $\boldsymbol{x}^0_J$, 
		%	$$b= B^0 +\sum_{a_i\in I} w_{a_i, b} a_i,$$ where $B^0$ is a constant which is the sum of the term including \boldsymbol{b} and $\boldsymbol{x}^0_J$. Applying Lemma \ref{lemma1}, we obtain the statement.
		%\end{proof}
		%
		...
		%
		%For any $b_h\in B_{pure}$, we have $b_h=\sum_{a_i\in I}w_{b_ha_i}+B_h$ ($B_h$ is the \boldsymbol{b}). 
		...
		%Hence we have: \begin{align*}
			...
			%	Suppose for layer up to $l_i$, this lemma is true, we show it for layer $l_{i+1}$. For a node $c$ in layer $l^{i+1}$, we have that: \begin{align*}
				%		\UB_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(c) \leq B_c + \sum_{b\in B_O\cap l_i} w_{b,c}\ReLU(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b))\\
			...
			%	
			%	
			%	But since $c\in B_{pure}$, for any $b\in B_O$, we must have $w_{b,c}=0$. And similarly, for any $b\in B_{zero}$, $w_{bc}=0$. Therefore, we will have:
			%	
			%	\begin{align*}
