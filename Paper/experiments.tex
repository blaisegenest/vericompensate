...
\begin{table}[]
	...
	\begin{tabular}{|c|c|cc||cc||c|}
		...
		\hline \hline
		$8\times200$\  &   $32\%$ & $73\%$ & $\mathbf{79.5\%}$ & {\color{blue}$75.5\%$}  & {\color{blue}$77\%$} & $91.5\%$ \\ 
		$\epsilon = 0.015$ & 17s & 93s & 451s & {\color{blue}391s} & {\color{blue}562s} &\\ \hline
	\end{tabular}
	\caption{$\%$ of verified images over 200 images. To see how refined BAB scale with TimeOut.
	We are not going to use it because on these 200 images, we dont do well ($4\%$ worse than on 1000 images), while $\alpha,\beta$-CROWN is similar.}
	\label{tab:example4}
\end{table}
