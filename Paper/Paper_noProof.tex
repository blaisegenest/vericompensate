...
\begin{algorithm}
	...
	Constant $\boldsymbol{W}, \boldsymbol{b}$ \tcp*{Weights and Bias of network}
	...
	\For{$l$ in Layers}{
		\If{$l$ = 0}{
			...
			$\mathcal{D}$, $\boldsymbol{W}, \boldsymbol{b}$ $\rightarrow$ $\mathcal{B}_0$ = \{$x$: $(UB(x),LB(x))$,$\cdots$\} 
		...
		\Else{
			...
			\For{$x$ in $Nodes_l$}{
				...
				$\mathcal{B}$, $\boldsymbol{W}, \boldsymbol{b}$, $\mathcal{O}$ $\rightarrow \mathcal{M}$  \tcp*{MILP model $\mathcal{M}$ for node $x$}
