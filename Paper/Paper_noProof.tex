...
%	\end{figure}
	...
	\begin{figure}[t!]
		...
		\begin{tikzpicture}
			...
			minimum height = 20] (input1) {$x_1$};
			
			\node[circle, draw= purple, thick, minimum width = 20,
			minimum height = 20] (input2) at ($(input1) + (0,-1.5)$) {$x_2$};
			...
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (hidden1) at ($(input1) + (2.5,0)$) {$x_3$};
			\node[circle, draw= black, thick] (hidden2) at ($(input1) + (2.5,-1.5)$) {$x_4$};
			
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (hidden3) at ($(input1) + (5,0)$){$\hat{x}_3$};
			\node[circle, draw= black, thick] (hidden4) at ($(input1) + (5,-1.5)$) {$\hat{x}_4$};
			
			
			\node[circle, draw= blue, thick, minimum width = 20,
			minimum height = 20] (hidden5) at ($(input1) + (7.5,0)$){$x_5$};
			\node[circle, draw= black, thick] (hidden6) at ($(input1) + (7.5,-1.5)$) {$x_6$};
			...
			% Output layer
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (output1) at ($(input1) + (10,0)$){$\hat{x}_5$};
			
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (output2) at ($(input1) + (10,-1.5)$){$\hat{x}_{6}$};
			...
			\draw[->,color=green, thick] (hidden1) -- (hidden3) node[midway, above] {$\max(x_1,0)$};
			\draw[->,color=red, thick] (hidden2) -- (hidden4) node[midway, above] {$\max(x_2,0)$};
			...
			\draw[->,thick] (hidden5) -- (output1) node[midway, above] {$\max(x_5,0)$};
			\draw[->,thick] (hidden6) -- (output2) node[midway, above] {$\max(x_6,0)$};
...
\begin{proposition}
	\label{prop.sign}
There exists two inputs $\vx^\star,\vx^\sharp$ such that:
\begin{enumerate} 
	\item ${\vx^\star}$ maximizes the value of all positive neurons and minimizes the value of all negative neurons.
	\item ${\vx^\sharp}$ minimizes the value of all positive neurons and maximizes the value of all negative neurons.
\end{enumerate}
\end{proposition}

Hence $\val_{\vx^\star}(z)=\beta$ and Hence $\val_{\vx^\sharp}(z)=\alpha$,
for $[\alpha, \beta]$ the bounds computed by the Box abstraction. We conclude Theorem \ref{th1} by invoking continuity.

\smallskip

Concerning the Theorem \ref{th2}, consider an output node $z$.
...
For that, we partition the input neurons from layer $0$ into four groups:
\begin{enumerate}
	\item $A_{zero}$ is the set of input neurons from where all paths with target $z$ have weight $0$.
	\item $A_{pos}$ is the set of input neurons from where all paths with target $z$ have weight $\geq 0$.
	\item $A_{neg}$ is the set of input neurons from where all paths with target $z$ have weight $\leq 0$.
	\item $A_{open}$ is the set of remaining input neurons.
\end{enumerate}

We define $A_{pure}=A_{pos} \cup A_{neg}$.
We then partition the set of neurons in hidden layers: 
\begin{enumerate}
	\item $B_{zero}$ is the set of neurones from where all paths with target $z$ have weight $0$.
...
Notice that the value of any node $n$ in $A_{pure}$ or $B_{pure}$ is entirely determined by 
nodes in $A_{pure}$ or $B_{pure}$. 
We can define the sign function for these neurons.
Therefore, applying Prop. \ref{prop.sign}, we obtain 
$\vx_{pure}^\star,\vx_{pure}^\sharp$ optimizing neurons in $A_{pure}$ or $B_{pure}$.
Now, assume that $\MILP_{X_z}$ computes bounds $[\alpha,\beta]$ for $z$.
We prove that there exists input 
...
$\val_{\vx^\star}(z)=\beta$ and $\val_{\vx^\sharp}(z)=\alpha$.
...
\begin{enumerate}
  \item Evaluate experimentally how accurate is $\MILP_Z$ when using compensating strength 
 to choose the set $Z$ of ReLU nodes encoded exactly using an integer variable, compared with randomly choosing the set $Z$, to make sure the choice is relevant. Also compare different choices for $K$ the size of $Z$ and how to rank nodes from different layers,
...
\subsection{Setting}
All the benchmarks were run on an AMD 5975WX ($32$ cores$@3.6$GHz, 7nm, year 2022) with 256 GB of main memory and 2 NVIDIA RTX 3090 (year 2022). Gurobi 9.52 was used for solving MILP and LP problems. Our implementations for Compensate($K$) is entirely realized in Python 
...
We experiment with the smallest network (MLP $6\times 100$), so that we can obtain exact bounds for the first few layers using a full MILP encoding of the DNN. We test over the $\vx=59$th image in the MNIST dataset, as it has a large number of unstable ReLU nodes in the first few layers ($61$ in the first layer and $55$ in the second layer), so we can experiment with a larger choice of values) for $K$.
...
A compensating pair $(\pi,\pi')$ of paths with target $n$ is thus of the form a node $a b c$ and $a b' c$, with $a$ an input node and $b,b'$ on the first layer. The compensating strength for $(b,b')$ is defined as 
...
We report in Table \ref{tab:example0} the average uncertainty of $\MILP_Z$ following the choice of the $K$ heaviest compensating ReLU nodes in $Z$, vs choosing $K$ nodes randomly. The range of uncertainty created by inaccurate computations is up to $1.17=2.22-1.05$, $2.22$ being the accuracy provided by LP, and $1.05$ by an exact MILP encoding of all the unstable ReLU nodes. Selecting $30$ out of the $61$ unstable ReLU nodes, the uncertainty created by inaccuracy is reduced by $80\%$, while a random choice of nodes only leads up to roughly halving this number. We can deduce that selecting nodes based on compensating strength helps to select important ReLU nodes for the accuracy.
...
\begin{itemize}
	...
	Notice that on these MLP networks, PRIMA resort to refined bounds on the first few layers (3 or 4) using exact MILP encoding of all the nodes (which is easily parallelized - 16 nodes considered in parallel in PRIMA). This is doable on the first few layers as the number of integer variable is limited (but still consequent). By comparison, we run MILP on all the layers, but with a bounded number of integer variables to control the runtime and to scale to all the layers.
	\item $\beta$-CROWN, with results as reported in \cite{crown}, as it is the most accurate of the efficient tool to date. Its main idea is to implement a very efficient Branch and Bound (BaB) verification tool, subsuming \cite{BaB} on such DNNs, and heavily using GPU implementation (while we do not). Performance of pure BaB on these hard DNNs is however impaired by the large number of branches to consider. Similarly as PRIMA, $\beta$-CROWN resorts to refining the bounds of the first few layers using an exact MILP encoding, also parallelized with 16 nodes in parallel, before running their BaB tool. We also test with the latest versions of $\alpha$-$\beta$-CROWN (October 2023)
...
\begin{table}[t!]
	...
	\begin{tabular}{|c||c|c|c||cc|cc||c|}
		...
		\hline
		\text{DNN}  & DeepPoly & PRIMA & $\beta$-CROWN &\multicolumn{2}{@{}c@{}|}{\text{ Comp (adapt.) }} & \multicolumn{2}{@{}c@{}|}{\text{ Comp (fixed) }} & Upper\\ 
...
More importantly, $\beta$-Crown, while efficient, often faces a (complexity) wall, when there number of branches becomes too extreme for BaB to work: either BaB succeeds fast, or it will not succeed even with very long time out (the reason why Refined BaB is more accurate on these networks). We verify that on a larger DNN learnt in a natural way, namely MLP $6 \times 500$, see Table \ref{tab:example3}.
We tested PureBaB with two time out, from the standard 30s to an extreme 2000s, to match the average run time of our adaptative implementation (fast mode). Only $3.5\%$ more of images could be ceritifed by extending the TO from 30s to 2000s, certifying $20\%$ less images than our prototype in the same time.
...
\begin{table}[b!]
	...
	\begin{tabular}{|c||c|c|cc||c||c|}
		...
		\hline
		\text{DNN}  & DeepPoly & PRIMA &  \multicolumn{2}{@{}c@{}|}{\text{ $\beta$-CROWN (pureBaB)}} & 
		Comp(adapt.) & Upper\\ 
		 & / CROWN &  & TO=30s & TO=1200s & fast & Bound\\
		\hline \hline
		
		
		$6\times500$\  &   $26\%$ & $32.5\%$ & $41\%$ & $44.5\%$  & $\mathbf{65\%}$ & $92\%$ \\ 

		$\epsilon = 0.035$ & 34s & 119s & 18.4s & 576s & 925s &\\ \hline
	\end{tabular}
...
Last, we report the average runtime per number of nodes of both fast adaptive and of fast fixed compensation, for the different DNNs we considered. The time per node is not constant, but it is still controlled and does not show an exponential behavior (from 0.1s per node for the smallest DNN to 0.3s per node for the biggest DNN). This indicates that Compensate($K$) can be run for larger DNN. While the time to verify images will be longer, it won't hit a wall as BaB does on DNNs hard to verify.
...
For DNNs easy to verify, $\beta$-CROWN and PRIMA are excellent solutions, both very fast and very accurate, usually matching or getting very close to the upper bound on the $\%$ of verified images. Compensation strength is not required in these contexts, and likely not useful, reason why we did not considered it.

We only considered DNNs with ReLU activation functions in this paper. The reason is that MILP, which is the basic block of Compensate($K$), and BaB (the basic block of $\beta$-Crown) can handle the ReLU activation function efficiently, and more complex activation functions are abstracted in other ways, which PRIMA handles (unlike $\beta$-CROWN). Still, the compensation paradigm is totally orthogonal to ReLU activation function. It is very likely that studying the compensation strength would be useful in this other context. We leave that for future work. 

While accuracy of compensate($K$) can be among the highest of the efficient verifiers, runtime, while never blowing-up, can be on the slow side, in particular for larger DNNs. There are many optimizations possible. The simplest is to run $\beta$-CROWN first in the pipeline, and only use Compensate($K$) for those images for which $\beta$-CROWN cannot conclude. Another simple way is to modify the refined BaB of $\beta$-CROWN to run compensate($K$) on more layers than what exact MILP can be run, and finish the study using BaB. More involved would be to modify BaB to branch on unstable ReLU nodes guided by the compensation strength, which we experimentally proved to be an efficient trade-off. Also, an analysis of important nodes for the uncertain output nodes could be performed, allowing to compute much faster less accurate bounds for those nodes not important. Last, one could perform a Refinement Abstraction framework similar to \cite{atva}, \cite{elboher}, or \cite{SRGR}.

On the correlation between neurons, which PRIMA encodes explicitly, our results tend to show that it is more efficient to use the compensation strength. Still, considering ReLU nodes exactly tends to be very accurate locally, but less when the source of correlation is many layers away. Few important correlations a la PRIMA could be kept explicitly as linear constraints into the MILP encoding, helping to reduce the imprecision further.
