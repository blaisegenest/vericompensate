...
The characteristic is that, the products of all weights in the paths, have two different signs: along $bc$, the product is (strictly) positive, while along $b'c'$, the product is (strictly) negative. This is key reason why fast approximation like LP cannot get the exact upper and lower bounds.

If both pairs are negative or positive, LP or even Interval Arithmetic will get the exact values of lower and upper bounds.
...
The following theorem shows the meaning of compensating pairs:
...
In this subsection, we will introduce the method to choose open nodes for MILP model by compensating pairs.
