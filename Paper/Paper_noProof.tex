...
%	\end{figure}
	...
	\begin{figure}[t!]
		...
		\begin{tikzpicture}
			...
			\node[circle, draw= blue, thick] (hidden6) at ($(input1) + (7.5,-1.5)$) {$x_6$};
			...
			\draw[->,thick] (hidden1) -- (hidden3) node[midway, above] {$\max(x_1,0)$};
			\draw[->,thick] (hidden2) -- (hidden4) node[midway, above] {$\max(x_2,0)$};
		...
		\end{tikzpicture}
		\caption{A DNN example from \cite{kneuron}, where every node is separated into 2, $x$ after the weighted sum and $\hat{x}$ after the ReLU activation function.}
		\label{fig1}
	\end{figure}
...
While there is an optimal linear function for the upper bound $g_i$ for ReLU nodes, there are two choices for the lower bound: $f_i(x_i) = 0$ or $f_i(x_i)=x_i$.
...
Compensating pair of paths will make precision of the target state reduce when using overabstraction of values. Here, $weight(x_5) \leq 4$ (the value is reached for $x_1=1$ and $x_2=-1$). However, Box computes an upper bound of $6$, and while DeepPoly recovers some of the dependency in $x_2$, it can only cancel out some of $x_2$ value due to the ReLU over-abstraction (a contribution of $\frac{-x_2+4}{2} \leq 2.5$ instead of the exact $-x_2 \leq 1.5$), and obtain a bound $weight(x_5) \leq 5.5$ instead of $4$.

Intuitively, the target state receives the weight from the source state from the different paths from the source to the target. As the weights of the paths is of opposite sign, some of these weights will cancel out,
by an amount up to $min(|weight(\pi')|,|weight(\pi')|)$. Further, no only do they cancel $min(|weight(\pi')|,|weight(\pi')|)$ out, but they also do not accumulate, in the same way as 
$0= |1+(-1)| \leq |1|+|-1|=2$ from an inacurate analysis. The amount of compensation can however be reduced by the ReLU functions, because of clipping by the ReLU function. For instance, consider that the inputs $x_1,x_2$ lies in $[-1,-0]$. Then the weight at $\hat{x}_3$ will be always 0, no matter the input, and there will be no compensation to $weight(\pi')$.
...
over all source/target states in given layers. The average compensations strenght are much lower in the Robust DNN than in the Natural DNN, and indeed the Robust DNN is much simpler to verify than the Natural DNN.
...
\begin{algorithm}[t!]
	\caption{}
	\label{algo1}
	\KwInput{Bounds $[\alpha_n,\beta_n]$ for input nodes $n$ at layer $0$}
	
	\KwOutput{Bounds $[\alpha_n,\beta_n]$ for every node $n$}
		 	
	 Run DeepPoly to obtain preliminary lower and upper bounds for all nodes.

	 Precompute $\mathcal{R}$ to know the compensation strenght
...
This algorithm has a worst case complexity bounded by $O(N 2^K LP(N))$, where $N$ is the number of nodes of the DNN, and $LP(N)$ is the (polynomial time) complexity of solving a linear program representing a DNN with $N$ nodes. This complexity is an upper bound, as e.g. Gurobi is fairly efficient and never need to consider all of the $2^K$ ReLU configurations to compute the bounds.





\section{Paths and Open Nodes Chosen}
