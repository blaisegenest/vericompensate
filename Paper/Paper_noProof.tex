...
We use $\boldsymbol{x}$ to denote the vector of input and  $f(\boldsymbol{x})$ to denote the output. Finally, neurons are also called \emph{nodes}, and when we refer to a specific neuron, we use $a,b,c,d$ to denote them, and $W_{a,b}$ to denote the weight from neuron $a$ to $b$. Similarly, for input $\boldsymbol{x}$, we denote by $weight_{\boldsymbol{x}}(a)$
the weight of neuron $a$ when the the input is $\boldsymbol{x}$.
...
Similarly, for simplicity, we focus on local robustness for the verification problem, altough generalization to other problems pose no problem. 
Local robustness asks to determine whether the output of a neural network will be affected under small perturbations to the input. In formula, if we use a distance $d$, and if the $\boldsymbol{x}_0$ is a ordinary input, then the considered domain $\mathcal{D}=\{x: d(\boldsymbol{x},\boldsymbol{x}_0)<\varepsilon\}$ where $\varepsilon$ is the parameter measuring the size of perturbation. And the problem will be \begin{align*}
