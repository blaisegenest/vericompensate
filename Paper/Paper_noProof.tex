...
\begin{table}[t!]
	...
	\begin{tabular}{|c||c|c|c||cc|cc||c|}
		...
		$5 \times100$\  &   $16\%$ & $51\%$ & $\mathbf{69.9\%}$ & no & point & $65.3\%$ & $68.4\%$ &  $84.2 \%$ \\ 
		$\epsilon = 0.026$ & 5s & 159s & 102s& & & 117s & 142s &  \\
		\hline	
...
leaving BaB with few ReLU nodes to branch on, and $\beta$-CROWN (refined BaB) is slightly more accuate than using compensation: we are within $1.5\%$ of the accuracy. On deeper DNNs ($8 \times 100$ and $8 \times 100$ with 8 hidden layers), the refinement with MILP cannot treat the 4 last layers, and BaB has more ReLU to branch on. Therefore, Compensate($K$) is more accurate than $\beta$-CROWN, 
