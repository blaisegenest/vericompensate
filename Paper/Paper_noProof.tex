...
%	\end{figure}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (hidden3) at ($(input1) + (5,0)$){$x_5$};
			\node[circle, draw= black, thick] (hidden4) at ($(input1) + (5,-1.5)$) {$x_6$};
			
			
			\node[circle, draw= blue, thick, minimum width = 20,
			minimum height = 20] (hidden5) at ($(input1) + (7.5,0)$){$x_7$};
			\node[circle, draw= blue, thick] (hidden6) at ($(input1) + (7.5,-1.5)$) {$x_8$};
			...
			% Output layer
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (output1) at ($(input1) + (10,0)$){$x_9$};
			
			\node[circle, draw= black, thick, minimum width = 20,
			minimum height = 20] (output2) at ($(input1) + (10,-1.5)$){$x_{10}$};
			...
			\draw[->,thick] (input2) -- (hidden1) node[near start, above] {$1$};
			...
			\draw[->,thick] (hidden4) -- (hidden5)node[near start, above] {$2$};
			\draw[->,thick] (hidden4) -- (hidden6)node[near start, below] {$1$};
			
			
			
			
			\draw[->,thick] (hidden5) -- (output1) node[midway, above] {$\max(x_7,0)$};
			\draw[->,thick] (hidden6) -- (output2) node[midway, above] {$\max(x_8,0)$};
...
With that, we get 
$x_7 \leq g_5(x_3) + 2 g_6(x_4) = \frac{x_3 + 2x_4 + 6}{2} = \frac{3x_1 - x_2 + 6}{2}\leq 5.5$.
...
\section{Compensating pair of paths}
...
Formally, a pair $(\pi,\pi')$ of paths is compensating if:
\begin{itemize}
 \item they have the same starting node $a$ (called {\em source} state) and ending node $b$ (called {\em target} state of $(\pi,\pi')$),
 \item they are disjoints,
 \item the weight $weight(\pi') < 0 < weight(\pi)$.
\end{itemize}

For instance, on Fig.~\ref{fig1}, the paths $\pi= x_2 x_3 x_7$ has weight $1$, while the
path $\pi'= x_2 x_4 x_7$ has weight $-2$, hence $(\pi,\pi')$ is a compensating pair of paths.
We call a path $\pi$ {\em compensating} if there exists a path $\pi'$ such that either $(\pi,\pi')$ or 
$(\pi',\pi)$ is compensating.

Compensating pair of paths will make precision of the target state reduce when using overabstraction of values. Here, $weight(x_7) \leq 4$ (the value is reached for $x_1=1$ and $x_2=-1$). However, Box computes an upper bound of $6$, and while DeepPoly recovers some of the dependency in $x_2$, it can only cancel out some of $x_2$ value due to the ReLU over-abstraction (a contribution of $\frac{-x_2+4}{2} \leq 2.5$ instead of the exact $-x_2 \leq 1.5$), and obtain a bound of $weight(x_7) \leq 5.5$ instead of $4$.

Intuitively, the target state receives the weight from the source state from the different paths from the source to the target. As the weight of the path is opposite, some of this weight will cancel out,
by an amount up to $2 min(|weight(\pi')|,|weight(\pi')|)$. This amount can however be reduced by the ReLU functions, because of clipping by the ReLU function. Assume that the inputs $x_1,x_2$ lies in $[-1,-0]$. Then the weight at $\hat{x_3}$ will be always 0, no matter the input, and there will be no compensation to 
$weight(\pi')$.
