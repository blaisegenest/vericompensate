...
We prove now that the LP abstraction is strictly more accurate that the DeepPoly abstraction, as it actually corresponds to considering in DeepPoly both linear functions lower bounding the ReLU function, $ReLU(n) \geq 0$ and $ReLU(n) \geq n$, while DeepPoly considers exactly one.

\begin{proposition}
	\label{LP}
 Given $n \in [\alpha,\beta]$ with $\alpha < 0 < \beta$, the following two systems of constraints 
 1) for LP and 2) for DeepPoly with both lower bounds are equivalent:
 \vspace{-0.3cm}
 \begin{align*}
	 & 1) \quad \hat{n} \geq n \quad \wedge \quad \hat{n} \geq 0, \quad \wedge \quad \hat{n} \leq \beta \cdot a \quad \wedge \quad \hat{n} \leq n-\alpha \cdot (1-a), \, a \in [0,1] \\
	 \text{and} \quad  & 2)  \quad \hat{n} \geq n \quad \wedge \quad \hat{n} \geq 0 \quad \wedge \quad \hat{n} \leq \beta \frac{n-\alpha}{\beta-\alpha}
\end{align*} 
\end{proposition}
...
It remains to prove that the 2 upper bound constraints of 1) LP corresponds to the 2) DeepPoly linear upper bound. For that, we study the upper bound function in the variable $a \in  [0,1]$, with $n \in [\alpha,\beta]$ fixed. We have $\hat{n}$ in 1) upper bounded by $max_{a \in [0,1]} (min(\beta \cdot a, n - \alpha (1-a)))$, and this bound can be reached. 
The function $min(\beta \cdot a, n - \alpha (1-a))$ reaches its maximum for 
$\beta \cdot a = n - \alpha (1-a)$, that is 
$(\beta - \alpha) a = n - \alpha$, i.e. 
$$ a = \frac{n - \alpha}{\beta-\alpha}$$

It thus gives an upper bound at 
$\beta \cdot a = \beta \frac{n - \alpha}{\beta-\alpha}$, which is exactly the function used in DeepPoly.
\hfill $\square$
\end{proof}
