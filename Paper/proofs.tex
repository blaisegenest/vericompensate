...
We prove by induction from the input layer to higher layers that the upper bound of node $n$ in $D'$, is greater than or equal to the upper bound in $D$; and similarly for the lower bound. We use  $\Val^{D}_{x_J^0}(n)$ to denote the range of possible values for node $n$ in MILP model of $D$ when $x_J=x_J^0$. This may be a single number or an interval. 

For any input $x_J=x_J^0$, we prove $\Val^{D'}_{x_J^0}(n) \geq \UB(\Val^{D}_{x_J^0}(n))$
by induction:


\begin{align*}
\Val^{D'}_{x_J^0}(n) = & \boldsymbol{bias}_n+\sum_{m\in B_{pure}} \max(\ReLU(W_{mn}m))+\sum_{m\notin B_{pure}} \ReLU(W_{mn}\Val^{D'}_{x_J^0}(m))\\
(\text{by induction})\geq & \boldsymbol{bias}_n+\sum_{m\in B_{pure}} \max(\ReLU(W_{mn}m))+\sum_{m\notin B_{pure}} \UB(\ReLU(W_{mn}\Val^{D}_{x_J^0}(m)))\\
= & \boldsymbol{bias}_n+\sum_{m\in B_{pure}} \UB(\mathrm{AppB}(W_{mn}))+\sum_{m\notin B_{pure}}  \UB(\ReLU(W_{mn}\Val^{D}_{x_J^0}(m)))\\
\geq & \UB(\Val^{D}_{x_J^0}(n)), \text{when } S(n)=1
\end{align*} 

Hence, using the $\max$, we will have $\max(z) = \beta'\geq \beta \geq \max(z)$, and hence $\beta=\beta'=\max(z)$.
