	...
	\begin{proof}
		...
		Therefore we have the following inequalities:\begin{align*}
				\max(\Val(c))\leq\sum_{b \in L_i, W_{bc n}>0}W_{bc} \max(\Val(\hat{b})) + \sum_{b \in L_i, W_{bc}<0}W_{bc} \min(\Val(\hat{b})) + \boldsymbol{bisa}_c\\
			\min(\Val(c))\geq\sum_{b \in L_i, W_{bc n}>0}W_{bc} \min(\Val(\hat{b})) + \sum_{b \in L_i, W_{bc}<0}W_{bc} \max(\Val(\hat{b})) + \boldsymbol{bisa}_c
		\end{align*}
