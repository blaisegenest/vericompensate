...
For convenience, when we discuss about the maximal or minimal values for nodes $n$ in a network without risk of confusing, 
we will write $\max(n)$ to denote $\max(\Val_{\vx}(n))$, and $\min(n)$ to denote $\min(\Val_{\vx}(n))$.
