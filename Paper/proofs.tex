...
For any input $x_J=x_J^0$, we prove $\beta'_{x_J^0} \geq \beta_{x_J^0}$ by induction on $n$:


\begin{align*}
	\beta'_{x_J^0}(n) = & {bias}_n+\sum_{m\in B_{pure}} W_{mn} \beta'(\hat{m})
	+\sum_{m\notin B_{pure}} W_{mn} (\beta'_{x_J^0}(\hat{m}))\\
(\text{by induction})\geq & {bias}_n+\sum_{m\in B_{pure}} \max(\ReLU(W_{mn}m))+\sum_{m\notin B_{pure}} 
\UB(\ReLU(W_{mn}\Val^{D}_{x_J^0}(m)))\\
= & {bias}_n+\sum_{m\in B_{pure}} \UB(\mathrm{AppB}(W_{mn}))+\sum_{m\notin B_{pure}}  \UB(\ReLU(W_{mn}\Val^{D}_{x_J^0}(m)))\\
