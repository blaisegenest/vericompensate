	...
	In general, $S$ may not be defined on every node (e.g. if there is a negative and a positive path from $a$ to $c$). However, if there is no compensating pair, $S$ is defined on all nodes: any node fulfills one of above cases (1),(2),(3).
		...
		\begin{enumerate}
			\item  $S(a_i)=0$ if 
			for all $b_j, weigth(a_i b_j)\cdot weigth(b_j c) = 0$
			
			
			\item  $S(a_i)=1$ if for all $b_j, weigth(a_i b_j)\cdot weigth(b_j c) \geq 0$ and there exists 
			$j, weigth(a_i b_j)\cdot weigth(b_j c) > 0$
			
			\item $S(a_i)=-1$ if for all $b_j\ weigth(a_i b_j)\cdot weigth(b_j c) \leq 0$ and there exists 
			$j, weigth(a_i b_j)\cdot weigth(b_j c) < 0$ 
		\end{enumerate}
		
	For $b_j$ in the hidden layer, we have $S(b_j)=1,-1,0$ if $weigth(b_j c)$ is positive, negative, or 0 respectively. Finally, for the output node $z$, we define $S(z)=1$.
	...
	\begin{lemma}[Sign]
		...
		$weigth(mn) \neq 0$ and $S(n) \neq 0$, then 
		$S(m)=S(n)\mathrm{Sign}(weigth(n m))$.
	\end{lemma}
	...
	\begin{proof}
		...
		$S(n)\mathrm{Sign}(weigth(n m))$. As there is no compensation, $S(m)=S(n)\mathrm{Sign}(weigth(n m))$.
		\hfill $\square$
	\end{proof}
	...
	\begin{proof}
		We prove this lemma by induction on layers. For input layer, this is true by definition. Suppose this lemma is true up to layer $L_i$, we need to show this for layer $L_{i+1}$. Let $c$ be a node in layer $L_{i+1}$, then by definition, we have the following equation: \begin{align*}
			c = \boldsymbol{bias}_c+\sum_{b\in L}weigth(bc)\ReLU(b).
		\end{align*}
		
		Therefore we have the following inequalities:\begin{align*}
				\max(c)\leq\sum_{b \in L_i, weigth(bc )>0}weigth(bc) \max(\hat{b}) + \sum_{b \in L_i, weigth(bc)<0}weigth(bc) \min(\hat{b}) + \boldsymbol{bisa}_c\\
			\min(c)\geq\sum_{b \in L_i, weigth(bc)>0}weigth(bc) \min(\hat{b}) + \sum_{b \in L_i, weigth(bc)<0}weigth(bc) \max(\hat{b}) + \boldsymbol{bisa}_c
		\end{align*}
		
		
		
		
		Now, if $S(c)=1$, we consider different case of $b$. If $S(b)=1$, then by Lemma \ref{lem:sign}, $weigth(bc)>0$; if $S(b)=-1$, then $weigth(bc)<0$; if $S(b)=0$, then $weigth(bc)=0$.
		...
		So combining formulas and above two facts, we will have that when the input vector is $\vx^*$, \begin{align*}
			\max(c)&\geq\Val_{\vx^*}(c) \\
			&= \sum_{b \in L_i, weigth(bc)>0}weigth(bc) \max(\hat{b}) + \sum_{b \in L_i, weigth(bc)<0}weigth(bc) \min(\hat{b}) + \boldsymbol{bisa}_c\\
			&\geq\max(c)
...
%		Then:
%		$$ S(n) \max(S(n) n)=\sum_{m \in L}weigth(m n) S(m) \max(S(m) \hat{m})+bias_n \text{ and }$$
%		$$S(n) \max(S(n)\hat{n}_s)=\ReLU(\sum_{m \in L}weigth(m n) S(n)\max(S(n)\hat{m}_s)+bias_n)$$
%		
%		Similarly for minimal value.
%		
%		%	for minimal value,	
%		%	$ \min(n)=\sum_{m \in L}W_{m n)\min(\)+bias_n $ and
%			%	$ \min(\hat{n}_s)=ReLU(\sum_{m \in L}weigth(m n)\min(\hat{m)s}+bias_n)$
%		\end{lemma}
%		
%		\begin{proof}
...
%			We have $\Val(b_s)= \Val(b) = \sum_{a \in L_0} weigth(ab) \Val(a) + bias_b$.
%			If every $weigth(ab) \Val(a)$ is maximized in the same time, 
%			then $\Val(b_s)= \Val(b)$ is also maximized. 
%			
%			We claim that this is the case at 
%			configuration $\boldsymbol{x}^*$.
%			Indeed, if $weigth(ab)=0$, any $\Val(a)$ can be chosen to maximize 
%			$weigth(ab)\Val(a)$. If $weigth(ab)>0$ then $S(a) = +1$ by Lemma \ref{lemma1}, since
%			$S(b)=+1$. We have $\max(a_s) = \max(a)$, maximizing $weigth(ab)\Val(a)$.
%			Finally, if $weigth(a b)<0$ then $S(a_s) = -1$, and we have 
%			$\max(a_s) = min(a)$, maximizing $weigth(ab)\Val(a)$.
%			
%			Hence $b$ reaches is maximum in configuration $\boldsymbol{x}^*$, and the value satisfies: 
%			$$\max(b)=\sum_{m \in L}weigth(a b) \max(a_s)+bias_b$$
%			
...
%			$\ReLU(\sum_{m \in L}weigth(a )\max(a_s)+bias_b)$, again uniformly reached over all $b$ at configuration $\boldsymbol{x}^*$.
%			
...
%		\end{proof}
		...
		\begin{corollary}
			\label{cor1}
			$$\max(c)=\sum_{b \in L, weigth(bc)>0}weigth(bc) \max(\hat{b}) + \sum_{b \in L, weigth(bc)<0}weigth(bc) \min(\hat{b}) + \boldsymbol{bias}_c$$
			$$\min(c)=\sum_{b \in L, weigth(bc)>0}weigth(bc) \min(\hat{b}) + \sum_{b \in L, weigth(bc)<0}weigth(bc) \max(\hat{b}) + \boldsymbol{bias}_c$$
		\end{corollary}
		...
		\begin{proof}
			...
			$$\bar{f}(n)= \sum_{weigth(mn)>0} weigth(mn) \bar{f}(\hat{m}) + \sum_{weigth(mn)<0} weigth(mn) \underline{f}(\hat{m}) + bias_n$$
		...
		%	\begin{itemize}
			...
			%	\begin{enumerate}
				%	 \item $weigth(b,z)=0$, or
				%	 \item For every input node $j\in J$, we have $weigth(a_j,b)=0$.
				%	\end{enumerate}
			%	We denote $B_{pure}$ the set of such nodes $b$ such that at least one of the above holds.
			%\end{lemma}
			%
			%\begin{proof}
			%	First, we show that if either one of 1,2 happens, then $b_i$ will not be opened. If 1), it is obvious. For 2, we reason by contradiction: assume there is a pair of compensating paths 	$(\pi,\pi')$ starting with $a$, with $k$ in $\pi$ and weight$(\pi) > 0$. It means that $a \in J$. A contradiction as 2) $weigth(a_j,b)=0$ implies weight$(\pi)=0$.
			%	
			%	Second, we show that if neither 1 nor 2 hold, then $b$ will be on a compensating path.
			%	Because 2) does not hold, there is a $j \in J$ with $weigth(a_j,b) \neq 0$, say $>0$.
			%	Because 1) does not hold, $weigth(b,z) \neq 0$, say $>0$.
			...
			%	$$b= B^0 +\sum_{a_i\in I} weigth(a_i, b) a_i,$$ where $B^0$ is a constant which is the sum of the term including bias and $\boldsymbol{x}^0_J$. Applying Lemma \ref{lemma1}, we obtain the statement.
			%\end{proof}
			%
			...
			%
			%For any $b_h\in B_{pure}$, we have $b_h=\sum_{a_i\in I}weigth(b_ha_i)+B_h$ ($B_h$ is the bias). 
			...
			%
			%\begin{align*}
			%	\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I} = B_z + \sum_{b\in B_{open}} weigth(b,z)\ReLU(weight_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)).
			%\end{align*} where $\mathrm{AppB}(weight_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b))$ is the upper bound of 
			%the (LP) approximation of $weight_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)$ if $weigth(b,z)>0$, and the lower bound
			%of its (LP) approximation if $weigth(b,z)<0$. 
			%
			%By Lemma \ref{lem:reach_max}, for any fixed input $\boldsymbol{x}^0_J$, for all nodes $b$ in the hidden layer, if $S(b)=\mathrm{sign}(weigth(zb))=1$, then 
			%$weight_{\boldsymbol{x}_I,\boldsymbol{x}^0_J}(b)$ will get its maximal value for $\boldsymbol{x}_I=\boldsymbol{x}_I^*$,
			%and if $S(b)=\mathrm{sign}(weigth(zb))=-1$, then $weight_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)$ will get its minimal value for $\boldsymbol{x}_I=\boldsymbol{x}_I^*$.
			%Notice that $\mathrm{ReLU}$ and $\mathrm{AppB}$ are both non decreasing functions, and thus:
			%
			%\begin{align*}
			%	\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I} \leq B_z + \sum_{b\in B_{open}} weigth(b,z)\ReLU(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) +
			%	\sum_{b\in B_{pure}} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)).
			%\end{align*} 
			%
			%Added FIX:
			%Consider $B_{pure}=B^1 \cup B^2$, with $b \in B^1$ iff
			%$weigth(b,z) = 0$ and $b \in B^2$ iff $weigth(b,z) \neq 0$.
			%Hence:
			%
			%$$\sum_{b\in B_{pure}} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) = 
			%\sum_{b\in B^2} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b))$$
			%
			%For $b \in B^2$, we have $weigth(a_j,b)=0$ for all $j \in J$
			%by definition of $B_{pure}$ (Lemma 4.), hence
			%
			%$$\sum_{b\in B^2} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) = 
			%\sum_{b\in B^2} weigth(b,z) \mathrm{AppB}(weight_{\boldsymbol{x}^*_I}(b))
			%=\sum_{b\in B^2} weigth(b,z) \mathrm{ReLU}(weight_{\boldsymbol{x}^*_I}(b))
			...
			%Hence we have: \begin{align*}
				%	\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}  \leq & B_z+\sum_{b}weigth(zb)\ReLU(b(\boldsymbol{x}_I^*,\bar{a}_J))
				...
				\begin{lemma} \label{lem:open_node_2}
					...
					\begin{enumerate}
					  	\item Any path from $b$ to the output node $z$ has a weight $0$, and we use $weigth(b,z)=0$ to denote this case, or
						\item For every input node $a_j\in J$, every path from $a_j$ to $b$ has a weight of $0$. We use $weigth(a_j,b)=0$ to denote this case.
					\end{enumerate}
				...
				\begin{lemma}\label{lem:subnetwork}
					$B_{pure}$ and $I$ can form a sub-network: every node in $B_{pure}\cup I$ only depends on nodes in $B_{pure}\cup I$ backward: if $b$ is connected to $c$ and $c$ is in $B_{pure}\cup I$ but $b$ is not, then $weigth(bc)=0$. We call this sub-network $D_I$. In $D_I$, there is no compensating pair.
				\end{lemma}
				...
				\begin{proof}
					...
					1. if $b\in J$, by definition, $weigth(bc)=0$.
					
					2. if $b\in B_{open}$, then $weigth(bc)=0$; otherwise there exists $<a_j,b,c>$ a path from $a_j\in J$ to $c$ with a nonzero value, a contradiction.
					
					3. if $b\in B_{zero}$, then $weigth(bc)=0$; otherwise there exists $<b,c,z>$ a path from $b$ to $z$ the output node with a nonzero value, a contradiction. \qed
				\end{proof}
				...
				\begin{lemma}
					If $c$ is in the next layer after $b$, $S(c)\neq 0, S(b)$ are both defined, and $weigth(bc)\neq 0$, then $weigth(bc)S(c)=S(b)$.
				\end{lemma}
				...
				\begin{proof}
					This is because if there exists nodes $b,c$ such that $weigth(bc)\neq 0$ and $c$ is in above set but $b$ is not, then $b$ has a path with nonzero weight from a node $a_i$ not in $J$ but this means so does $c$, a contradiction. Therefore any $b$ such that $weigth(bc)\neq 0$ is also in above set. \qed
				\end{proof}
				...
				%	Suppose for layer up to $l_i$, this lemma is true, we show it for layer $l_{i+1}$. For a node $c$ in layer $l^{i+1}$, we have that: \begin{align*}
					%		\UB_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(c) \leq B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b))\\
					%		 + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b))\\
					%		 +\sum_{b\in B_{zero}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)).
					%	\end{align*} Here, $\B$ means one of $\UB$ or $\LB$ depending on the weight/coefficient.
				%	
				%	
				%	But since $c\in B_{pure}$, for any $b\in B_{open}$, we must have $weigth(b,c)=0$. And similarly, for any $b\in B_{zero}$, $weigth(bc)=0$. Therefore, we will have:
				%	
				%	\begin{align*}
					%		\UB_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(c) \leq B_c + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)).
					%	\end{align*} By induction, for any $b\in B_{pure}\cap l^i$, it only depends on $\boldsymbol{x}^0_I$.  Therefore, we will have:
				%	
				%	\begin{align*}
					%		\UB_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(c) = B_c + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(B_{\boldsymbol{x}^0_I}(b)).
				...
				\begin{proof}
					...
					If $S(c)$ is defined, then consider all $b$ such that $weigth(bc)\neq 0$. First, $b$ cannot in $B_{zero}$; so by induction hypothesis, the value of $b$ is fixed under $\boldsymbol{x}^*_I,\boldsymbol{x}^0_J$ in MILP model. 
					
					
					
					If $b\in B_{open}$, the term is $weigth(bc)\ReLU(b)$. We know then by induction hypothesis, the value for $b$ in MILP is a fixed value. Then the term $weigth(bc)\ReLU(b)$ is also a fixed number.  
					
					If $b\in B_{pure}$, the term is $weigth(bc)\mathrm{AppB}(b)$ ($\mathrm{AppB}$ is the LP approximation domain of $\ReLU$) and either $S(b)$ is undefined and $b$ is a constant in the network and so is in the MILP model (trivial case), or $S(b)$ is defined. 
					
					Then by induction hypothesis for all 1, 2 and 3 parts,  the value of $b$ in the model is fixed (part 1), and it is equal to $\max_{\boldsymbol{x}_I} \Val_{\boldsymbol{x}_I,\boldsymbol{x}^0_J}(c)$ or $\min_{\boldsymbol{x}_I} \Val_{\boldsymbol{x}_I,\boldsymbol{x}^0_J}(c)$ (by part 2, depending on $S(b)$). Since $b$ only depends on $I$, this value also equal to $\max_{\boldsymbol{x}_I} \Val_{\boldsymbol{x}_I}(c)$ or $\min_{\boldsymbol{x}_I} \Val_{\boldsymbol{x}_I}(c)$, and by induction hypothesis on part 3, equal to $\UB(b)$ or $\LB(b)$. For either of them, $\mathrm{AppB}(b)$ will be fixed value. Hence, in this case, the term $weigth(bc)\mathrm{AppB}(b)$ is also a fixed number.
					...
					(2) If $S(c)=1$, then we have the following:	\begin{align*}
						\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}(c) \leq constant + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)),
					\end{align*}where $\B$ means one of $\UB$ or $\LB$ depending on the sign of $weigth(bc)$: if $weigth(bc)$ is positive, then $\UB$, otherwise $\LB$. Notice that all $b$ displayed are that $S(b)$ defined, because if $S(b)$ undefined, then for fixed $\vx_J$, value of $b$ is fixed and will be put into the constant.
					
					Now by induction hypothesis part 2, if $S(b)=1$, then $\UB_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)=\max_{\vx_I}\UB_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)$; and if $S(b)=-1$, $\LB_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)=\min_{\vx_I}\LB_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)$. Moreover, since $S(c)=1$, $S(b)$ and $weigth(bc)$ have the same sign. And both $\ReLU$ and $\mathrm{AppB}$ are non-decreasing functions. Combining all these facts, we have:\begin{align*}
						\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}(c)\leq 
						&constant + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^*_I}(b))\\
				...
				%	
				%	\begin{align*}
					%		\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}(c) \leq B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)).
					%	\end{align*} where $\mathrm{AppB}$ is the same as in previous section  whose definition is independent with $\boldsymbol{x}^0_I,\boldsymbol{x}^0_J$. $\B$ is one of $\UB,\LB$  that, if $weigth(b,c)>0$, then $\B$ is $\UB$ and otherwise $\B$ is $\LB$.
				%	
				%	We can simplify above equation to \begin{align*}
					%			\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}(c) \leq B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^0_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^0_I}(b)).
					%	\end{align*}
				%	
				%	By induction hypothesis, since both $\ReLU$ and $\mathrm{AppB}$ are non-decreasing functions, we have that \begin{align*}
					%		\mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^0_I}(c)\leq 
					%		 &B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(\B_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}\wedge weigth(b,c)\neq 0} weigth(b,c) \mathrm{AppB}(\B_{\boldsymbol{x}^*_I}(b))\\
					%		= & \mathrm{UB}_{\boldsymbol{x}^0_J,\boldsymbol{x}^*_I}(c) \\
				...
				%Now, since when $x$ reaches the max or min of $\mathrm{AppB}$'s domain, $\mathrm{AppB}(x)=\ReLU(x)$, and by induction hypothesis, for any $b\in B_{pure}\cap l_i$, $\mathrm{AppB}(B_{\boldsymbol{x}^*_I}(b))=\ReLU(B_{\boldsymbol{x}^*_I})$. Hence, by induction hypothesis, we will have \begin{align*}
					...
					%	= &B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(B_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}\wedge weigth(b,c)\neq 0} weigth(b,c) \mathrm{AppB}(B_{\boldsymbol{x}^*_I}(b))\\
					%	= &B_c + \sum_{b\in B_{open}\cap l_i} weigth(b,c)\ReLU(weight_{\boldsymbol{x}^*_I,\boldsymbol{x}^0_J}(b)) + \sum_{b\in B_{pure}\cap l_{i}} weigth(b,c) \mathrm{ReLU}(weight_{\boldsymbol{x}^*_I}(b))\\
