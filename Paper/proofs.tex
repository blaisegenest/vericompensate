...
\section{Proof of Theorem \ref{th2}}
			...
			For an input $\vx$ and a subset $S$ of input nodes, we denote by $\boldsymbol{x}_S$ to refer the input vector $\langle x_{a_k}\rangle_{a_k\in S}$. We use $\boldsymbol{x}_I\oplus \boldsymbol{x}_J = \boldsymbol{x}$ and $\Val(z)=\Val_{\boldsymbol{x}}(z)=\Val_{\boldsymbol{x}_I,\boldsymbol{x}_J} (z)$. 
			For a node $b$ for which $S$ is undefined, $b$ only depends on $J$; so, if $\boldsymbol{x}_J$ is fixed, then the value of $b$ will be fixed.
			...
			\begin{lemma} \label{lem:reach_max_2}
				Let $b$ be an output node or a node in a hidden layers such that $S(b)=1$. Then for any valuation $\boldsymbol{x}^0_J$,	we have: 
				$$\max_{\boldsymbol{x}_I} (\Val_{\boldsymbol{x}_I,\boldsymbol{x}^0_J}(b)) =  \Val_{\boldsymbol{x}_I^*,\boldsymbol{x}^0_J}(b).$$
				
				If $S(b)=-1$, then a similar results hold, replacing $\boldsymbol{x}^*_I$ by $\boldsymbol{x}^\sharp_I$. The opposite holds for $\min_{\boldsymbol{x}_I}$.
			\end{lemma}
			
			\begin{proof}
				
				
				For any fixed input vector $\boldsymbol{x}^0_J$, consider the DNN $D'$ with input nodes $a_i\in I$, with all $\boldsymbol{x}^0_J$ 
				%and their propagation in hidden layers 	
				as bias. We denote $\val'$ values of nodes In this DNN $D'$.
				 We have $\Val_{\boldsymbol{x}_I,\boldsymbol{x}^0_J}(z) = \val'_{\vx}(\boldsymbol{x}_I)(z)$.
				
				In the simplified $D'$, there is no compensating path because of the definition of $I$. Therefore we can apply Lemma \ref{lem:key1} to get that $\Val_{x_I,\boldsymbol{x}^0_J}(z)$ reaches its maximal value for $\boldsymbol{x}_I=\boldsymbol{x}_I^*$, and we are done. \qed
			\end{proof}
			
			From Lemma \ref{lem:reach_max_2}, we have the following corrolary:
			...
			Let $[\alpha,\beta]$ be the bounds computed by $\MILP_{X}$ for the target node $z$.
...
\iffalse			
		...
		We want to show that $\beta= \max(z)$. As the MILP abstraction is a sound over-approximation, it suffices to show that $\beta\leq \max (c)$.
...
\fi
			...
			Cut $P,P'$ into segments $P_0,P_1,\cdots$, and $P'_0,P'_1,\cdots$ at each common node; hence each $P_i,P_i'$ either have not common nodes (except the start and end nodes), or are the same. Now, for node $a$, there are three cases:
			\begin{itemize}
