...
\section{Conclusion}
\vspace{-0.2cm}
In this paper, we tackled {\em real-time} (few ms) certification for robustness, which has never been performed before to the best of our knowledge. The main idea is to proceed in 2 steps: first, compute {\em once, offline} global robustness bounds $\bar{\beta}$ on the DNN. Then use a very simplistic look-up table based on $\bar{\beta}$ to perform the {\em online} certification. 
...
Second, we developed the new {\em Diff} MILP method, which is more accurate for global robustness than the classical MILP encoding. This results in $25\%$ lower bounds in average than the state-of-the-art ITNE model \cite{ITNE}. 
