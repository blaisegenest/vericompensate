...
\section{Appendix for Section 4: Architecture of LAE-DNNs}
\label{app.lae.folding}

Section~4 constructs an AE-DNN by applying an autoencoder before the
original DNN. For the linear autoencoders considered in this paper,
the encoder-decoder transformation does not need to be implemented as
additional layers during exploitation: it can be absorbed into the
first linear layer of the original DNN. We formalize this observation
below.

Let the encoder and decoder of the LAE be given by
\begin{align*}
	\operatorname{Enc}(\boldsymbol{x})
	&=
	\boldsymbol{W}^{\mathrm{enc}}\boldsymbol{x}
	+\vb^{\mathrm{enc}}, \\
	\operatorname{Dec}(\boldsymbol{u})
	&=
	\boldsymbol{W}^{\mathrm{dec}}\boldsymbol{u}
	+\vb^{\mathrm{dec}},
\end{align*}
where
\[
\boldsymbol{W}^{\mathrm{enc}}
\in \mathbb{R}^{n\times d_0},
\qquad
\boldsymbol{W}^{\mathrm{dec}}
\in \mathbb{R}^{d_0\times n}.
\]
Their composition is the affine transformation
\begin{align*}
	T(\boldsymbol{x})
	&=
	\operatorname{Dec}(\operatorname{Enc}(\boldsymbol{x})) \\
	&=
	\boldsymbol{W}^{\mathrm{dec}}
	\boldsymbol{W}^{\mathrm{enc}}\boldsymbol{x}
	+
	\boldsymbol{W}^{\mathrm{dec}}\vb^{\mathrm{enc}}
	+
	\vb^{\mathrm{dec}}.
\end{align*}

\begin{proposition}
	\label{prop.lae.folding}
	Let $f$ be an $\ell$-layer DNN with weight matrices
	$\boldsymbol{W}^i$ and bias vectors $\vb^i$, as defined in
	Section~\ref{s.notation}. The LAE-DNN
	\[
	f_{\mathrm{LAE}}(\boldsymbol{x})
	=
	f(T(\boldsymbol{x}))
	\]
	can be represented by an $\ell$-layer DNN having the same layer
	dimensions and ReLU activations as $f$. It suffices to replace the
	weight matrix and bias vector of the first layer.
\end{proposition}

\begin{proof}
	For an input $\boldsymbol{x}$, the pre-activation values of the first
	layer of the LAE-DNN are
	\begin{align*}
		\boldsymbol{z}_{\mathrm{LAE}}^1
		&=
		\boldsymbol{W}^1T(\boldsymbol{x})+\vb^1 \\
		&=
		\boldsymbol{W}^1
		\left(
		\boldsymbol{W}^{\mathrm{dec}}
	...
	Define the new weight matrix and bias vector of the first layer by
	\begin{align*}
	...
	and consequently
	\[
	\hat{\boldsymbol{z}}_{\mathrm{LAE}}^1
	=
	\ReLU\left(
	\widetilde{\boldsymbol{W}}^1\boldsymbol{x}
	+
	...
	For every subsequent layer $i\in\{2,\ldots,\ell\}$, we keep
	\[
	\widetilde{\boldsymbol{W}}^i=\boldsymbol{W}^i,
	\qquad
	\widetilde{\vb}^i=\vb^i.
	\]
	Since the post-activation values of the first layer are identical,
	it follows inductively that the pre-activation and post-activation
	values of all subsequent layers are also identical. In particular,
	the output $\boldsymbol{z}^{\ell}$ is unchanged.
	...
	\qquad
	\widetilde{\vb}^1\in\mathbb{R}^{d_1}.
	\]
	Thus, folding the LAE transformation into the first linear layer
	changes neither the number of layers nor their dimensions. The
	resulting DNN computes exactly the same function as the explicit
	composition of the LAE and the original DNN.
\end{proof}

If the encoder and decoder contain no bias terms, the construction
simplifies to
\[
\widetilde{\boldsymbol{W}}^1
=
