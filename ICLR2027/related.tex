...
Some works focus on training DNNs to be more globally robust, either through dedicated architectures, albeit with lower accuracy \citep{Leino,Zhang22a,Sun22,Singla22}, or do not consider (non-ID) inputs \citep{REGLO}. Further, \citet{Leino,Chen21,Singla22} do not consider $L_\infty$ or $L_1$ norm. 
...
Our LAE-DNN approach does not require any change of architecture and thus accuracy change is limited (see Section~\ref{sec.pca}).  %cis to postprocess DNNs to obtain better global bounds using (linear) auto encoders
