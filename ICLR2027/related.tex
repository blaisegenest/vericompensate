...
%
%\eca{Some works rely on specialized architectures or dedicated robust training procedures that offer better global bounds, but suffer from lower clean accuracy \citep{Leino,Zhang22a,Sun22,Singla22, Chen21}. However, these methods are also frequently constrained by specific thread models: \citet{Singla22} is restricted to the $L_2$ norm, \citet{Zhang22a} to the $L_{\infty}$ norm, while \citet{Chen21} considers non-$L_p$ security properties. \citet{REGLO} provides a post-hoc DNN repair method and is thus restricted to local/in-distribution (ID) regions rather than the full-input domain.
...
Some works focus on training DNNs to be more globally robust through dedicated architectures, \citep{Leino,Zhang22a,Sun22,Singla22}, 
which results in drop in accuracy. 
Our LAE-DNN approach does not require any change of architecture and thus accuracy change is limited (see Section~\ref{sec.pca}).
Further, \citet{Leino,Chen21,Singla22} do not consider $L_\infty$ or $L_1$ norm. Last, \citep{REGLO} considers restricted input space.  
%A key aspect of global robustness involves ensuring that analyzed data is both semantically meaningful and in-distribution (ID).
  %cis to postprocess DNNs to obtain better global bounds using (linear) auto encoders
