...
%
Some works focus on training DNNs to be globally robust. Specifically, \cite{Leino,Zhang22a,Sun22,Singla22} propose special networks to be learnt with reasonable global bounds, but reduced accuracy.
Further, \cite{Leino,Chen21,Singla22} do not consider $L_\infty$ or $L_1$ norm, while \cite{REGLO} do not consider (non-ID) inputs.
...
Our approach is to postprocess DNNs to obtain better global bounds using(linear) auto encoders~\cite{hinton2006reducing}.
The (L)AE-DNN architectures (Fig.~\ref{fig:AE-DNN} in Section \ref{sec.pca}) project the input on a latent space to filter out non-ID features, with limited loss of accuracy, and it can be applied to these other techniques as postprocessing.
...
%More related, s
Others \cite{Marabou,lipshitz,ITNE,GROCET} consider certifying restricted Lipschitz bound, to understand how the output can change given an input perturbation. However, they do not consider whether the classification changes. 
%
...
ITNE \cite{ITNE} adds explicitly one {\em linear} constraint (Eq. (3) in \cite{ITNE}) on the {\em diff variables} $y,\hat{y}$,
%$\frac{y_i-\gamma_i}{2} \leq \hat{y}_i \leq \frac{y_i+\gamma_i}{2}$ 
implied by each of our MILP models {\em Diff}, {\em 2b+1} and {\em 1b} models. Vhagar \cite{vhagar} instead adds per neuron constraints (depending on the perturbation) between the (binary) variables to simplify the MILP problem, which is well-adapted to occlusion and patches but less to $L_1$-perturbations.
