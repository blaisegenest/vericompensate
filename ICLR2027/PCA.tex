...
Global upper bounds $\bar{\beta}$ obtained across the entire input space tend to be overly pessimistic on vanilla DNN, since they must accommodate all possible inputs, including inputs {\em not} In (training) Distribution (ID) \cite{OOD}. Such samples display large value for 
$x_C - x'_C + x'_D - x_D$, see Fig.~\ref{fig3} (left) for MNIST.
Considering the full input space on the original DNN will thus produce high global upper bounds $\bar{\beta}$, no matter the method employed to compute the bound (e.g., MILP). Restricting to ID inputs seems impractical, as there is no established characterization of ID inputs. We propose an efficient alternative:
...
Extract the most important components of the ID dataset using an Auto-Encoder (AE), and filter out the other components from an input before feeding it to the DNN. In practice, we train the AE on the ID dataset to obtain the ID latent space. We then define the {\em AE-DNN architecture} (Fig.~\ref{fig:AE-DNN}): first, use AE-encode to reduce to the latent space, then use AE-decode 
...
%
In the following, we consider {\em Linear} AE-DNN ({\em LAE-DNN}), as the MILP model for {\em non-linear} AE-DNN needs additional integer variables wrt the original DNN or LAE-DNN.
