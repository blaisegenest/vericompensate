...
Global upper bounds $\bar{\beta}$ obtained across the entire input space tend to be overly pessimistic on vanilla DNN, since they must accommodate all possible inputs, including inputs {\em not} In (training) Distribution (ID) \cite{OOD}. Such samples display large value for $x_C - x'_C + x'_D - x_D$, see Fig.~\ref{fig3} (left) for MNIST. So no matter what the method employed to compute them, the  upper bounds $\bar{\beta}_{C,D}^{\varepsilon}$ will be overly large as they have to be at least as large as the values obtained for these samples. 
...
We propose an efficient  {\em alternative} to use the original vanilla DNN: Construct an AE-DNN, described hereafter, 
and use it in exploitation instead of the original DNN.
It keeps very similar behavior on ID inputs (which is what matters in exploitation), but differs vastly on non ID-inputs (which is not important in exploitation) to avoid the large bounds $\bar{\beta}_{C,D}^{\varepsilon}$, and thus AE-DNN is more amenable to real-time certification.
...
To do so, we extract the most important components of the ID dataset using an Auto-Encoder (AE), and filter out the other components from an input before feeding it to the DNN. In practice, we train the AE on the ID dataset to obtain an ID-compatible latent space. We then define the {\em AE-DNN architecture} (Fig.~\ref{fig:AE-DNN}): first, use AE-encode to reduce to the latent space, then use AE-decode 
produce the {\em projected input} (in full space), 
and finally call DNN on this projected input. 
%Such architecture has been used previously, e.g. by \cite{MEnet}, but for a different purpose.

%AE-DNN will behave largely as the original DNN on ID inputs. However, 
%
In the following, we consider {\em Linear} AE-DNN ({\em LAE-DNN}), as the MILP model for {\em non-linear} AE-DNN would be more complex, needing additional integer variables wrt the original DNN or LAE-DNN.
...
An optimal choice of LAE-encode is the projection of the input to these $n$ vectors. That is, calling LAE-decode and LAE-encode filters out features not in the latent space.
