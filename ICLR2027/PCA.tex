...
%
In the following, we consider {\em Linear} AE-DNN ({\em LAE-DNN}), as the MILP model for {\em non-linear} AE-DNN would be more complex, needing additional integer variables wrt the original DNN.
...
Call $dim$ the dimension of the full space, and $n$ the size of the latent space of the LAE trained on an ID dataset. LAE-decode is a linear function. By optimality, it can be assumed to be of rank $n$: it defines $n$ linearly independent vectors of dimension $1 \times dim$.
...
Hence, on ID inputs, LAE-DNN will behave largely as the original DNN. However, global bounds on the LAE-DNNs will be smaller than on the original DNN, because of the projection to the latent space.
Indeed, largest $x_C - x'_C + x'_D - x_D$ found 
on LAE-DNN is 6 times smaller than on DNN, and corresponds to more ID images; see Fig.~\ref{fig3} (right). \eca{In addition, the LAE-DNN can have the same architecture as the original DNN: it suffices to absorb its linear transformation into the DNN's first layer's weights (see Appendix XXX).}
