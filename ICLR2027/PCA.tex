\section{(Linear) Auto-Encoders to project on  ID-compatible subspace}

\label{sec.pca}

Global upper bounds $\bar{\beta}$ obtained across the entire input space tend to be overly pessimistic on vanilla DNN, since they must accommodate all possible inputs, including inputs {\em not} In (training) Distribution (ID) \cite{OOD}. Such samples display large value for $x_C - x'_C + x'_D - x_D$, see Fig.~\ref{fig3} (left) for MNIST. So no matter what the method employed, the computed upper bound
$\bar{\beta}_{C,D}^{\varepsilon}$ will be overly large. 
...
Construct an AE-DNN, described hereafter, 
