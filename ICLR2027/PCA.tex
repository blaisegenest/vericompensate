...
{\color{red}We propose an efficient alternative to the original vanilla DNN: Construct the following AE-DNN and use it in exploitation instead. AE-DNNs exhibit very similar behavior on ID-inputs, which is critical during exploitation, but differ vastly on {\em non} ID-inputs to avoid large bounds $\bar{\beta}_{C,D}^{\varepsilon}$. They also have similar accuracy to vanilla DNNs, while being more amenable to real-time certification.}
