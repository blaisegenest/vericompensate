...
We propose an efficient alternative to the original vanilla DNN: 
{\color{red}Construct AE-DNNs as follows and use them in exploitation instead. AE-DNNs exhibit very similar behavior on ID-inputs, which is critical for exploitation, but can differ vastly on {\em non} ID-inputs, to avoid large bounds $\bar{\beta}_{C,D}^{\varepsilon}$. Hence, AE-DNN has similar accuracy as DNN, while being more amenable to real-time certification.}
