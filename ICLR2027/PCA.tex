...
We propose an efficient alternative to the original vanilla DNN: Construct AE-DNNs as follows and use them in exploitation instead. AE-DNNs exhibit very similar behavior on ID-inputs, which is critical for exploitation, but can differ vastly on {\em non} ID-inputs to avoid large bounds $\bar{\beta}_{C,D}^{\varepsilon}$. Hence, AE-DNNs achieve accuracy similar to vanilla DNNs while being more amenable to real-time certification.
...
%
To do so, we train an Auto-Encoder (AE) on the ID dataset, obtaining an ID-compatible latent space. We then define the {\em AE-DNN architecture} (Fig.~\ref{fig:AE-DNN}): first, AE-encode reduces to the latent space, then AE-decode produces the {\em projected input} (in full space), and last, call the DNN on this projected input. 
...
%
In the following, we consider {\em Linear} AE-DNN ({\em LAE-DNN}), as the MILP model for {\em non-linear} AE-DNN needs additional (expensive) integer variables wrt (LEA-)DNN.
Further, the original DNN architecture 
can be {\em retained} by absorbing the LEA layers of LEA-DNN into the DNN first layer's weights (Appendix~\ref{app.lae.folding}).
