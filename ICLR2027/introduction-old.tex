...
%
Unlike local robustness, which requires $k$ separate, computationally-expensive calls to certify $k$ images, the global upper bound $\bar{\beta}^\varepsilon_{C,D}$ is computed offline once per network and remains valid across the entire input space. 
...
\begin{enumerate}
	...
	\item {\em The AE-DNN architecture.} For vanilla DNNs, global bounds are particularly pessimistic (see Section \ref{sec.pca}), because inputs far away from the training dataset
...
for vanilla DNNs. 
%While explicitly {\em characterizing} ID inputs to restrict the analysis to them is impractical, 
We propose %an efficient alternative: 
to use the AE-DNN architecture (Fig.~\ref{fig:AE-DNN} from 
Section~\ref{sec.pca}) {\em in exploitation}, as an alternative
to the original vanilla DNNs. The AE-DNN first uses an auto-encoder (AE) learnt from ID samples, so that the large input space is projected to a small latent space which aligns well with ID samples. It is then projected back to full space before being input to the original DNN. 
%In this way, the AE-DNN characteristics are close to the original DNN on ID-input.
Non-ID samples will be projected by the AE-DNN to the ID-compatible latent space, so that they do not create artificially large global bounds.
We select the size of the latent space to ensure that the accuracies (which by definition only cares about ID inputs) of the AE-DNN and DNN are within $1$ \%. 
Last, we propose to use  {\em in exploitation} the AE-DNN rather than the original DNN, as their characteristics on ID-inputs are similar but 
global bounds are much better, enabling real-time certification for AE-DNNs.
%, meaning that the ID directions selected are sufficient for the DNN task.
We experimented with Linear AEs (LAE): While the encoding may not be as efficient as with non-Linear AE, the latter would significantly increase the computational cost. The latent space is $>30$x smaller 
than full space on standard datasets, leading to bounds $\bar{\beta}^\varepsilon_{C,D}$ $3$-$15$x smaller than for vanilla DNNs.
...
In terms of perturbations, we consider conjunctions of $L_\infty$- and $L_1$-norms, which allow to accurately describe perturbations. For instance, "each subpixel is perturbed by at most $\frac{50}{255}$ ($L_\infty$) and the sum of the absolute value of perturbations over all subpixels is at most $1$" ($L_1$-perturbation). 
...
(VHAGaR \cite{vhagar} and ITNE \cite{ITNE}),
...
%Optimizations are namely reducing the number of binary variables (VHAGaR \cite{vhagar}) or adding linear constraints (ITNE \cite{ITNE}). 
...
on LAE-DNNs used in exploitation, for an $L_1$-perturbation of $1,4,2$ over the MNIST, Fashion-MNIST, and CIFAR-10 datasets, respectively, see Table \ref{table.cert}. The online process only adds  $\approx 0.5$ ms of latency per image, and $\approx 2000$ images/second.
% can be treated per CPU core.
