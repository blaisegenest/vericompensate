...
\label{sec.exp}
	...
	ITNE \cite{ITNE} was reimplemented using the same path as our code, except for the MILP variable $x_j$ using the classical MILP encoding plus the extra linear constraint Eq.~(\ref{eq.lpr}). The Julia VHAGaR code 
	...
	from \cite{vhagar} ("conv1"). We will also consider 3 LAE-DNNs, see Section~\ref{s62}.
...
Recall that LAE-DNNs are generated following the process described in Section 5: for each latent space dimension, we report in Table \ref{table.LAE} the accuracy loss from using LAE-DNN vs using the original DNN. 
