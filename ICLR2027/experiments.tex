...
\begin{table}[b!]
    ...
    \\
	 & & ($\%$ of gap $\downarrow$) & (72$\%$) &  (9$\%$) & ({\bf \em 1}$\%$) & ({\em 9}$\%$) & ({\bf \em 0.6}$\%$)
    \\\hline
    ...
    \\
     ...
     \end{tabular}
	\caption{Upper bound $\bar{\alpha}$ and (gap to lower bound LB $\underline{\alpha}$), lower is better, for $L_\infty=0.05$ perturbation, averaging bounds $\bar{\alpha}_{C,D}$ over all pair of classes $(C,D)$. {\em Best} reports the average $\bar{\alpha}$ using the lowest $\bar{\alpha}_{C,D}$ computed 
	in parallel by {\em Diff} and {\em 2b+1} for each $(C,D)$. Lower gaps using {\em Best} means that there are cases $(C,D)$ where {\em Diff} is much better than {\em 2b+1}, and cases where it is the opposite. * means that VHAGaR failed to compute bounds for some cases $(C,D)$ of CIFAR-Conv. 
	For such cases, we used numbers from ITNE instead (which is overperforming VHAGaR in every test), so these numbers are an optimistic evaluation for VHAGaR.}
	\vspace{-0.4cm}
...
\label{sec.exp}
	...
	We implemented our code in Python 3.8 and Gurobi 9.52 to solve the MILP problems. We conducted our evaluation on an AMD Threadripper 7970X ($32$ cores$@4.0$GHz) with 256 GB of main memory. %and 2 NVIDIA RTX 4090 (not used, Gurobi is CPU only). 
	ITNE \cite{ITNE} was reimplemented using the same path as our code: the MILP variable $x_j$ uses the classical MILP encoding plus the extra linear constraint Eq.~(\ref{eq.lpr}). VHAGaR's code 
	from \url{https://github.com/ananmkabaha/VHAGaR} was run on the same hardware.
	
	We consider 4 pretrained Vanilla DNNs on 3 different datasets (MNIST, Fashion-MNIST, and CIFAR10). Namely,  MNIST-FC, a fully-connected DNN from \url{https://github.com/eth-sri/eran} ("5x100"), and 3 convolutional DNNs: MNIST-Conv, FMNIST-Conv, and CIFAR10-Conv, 
	from \cite{vhagar} ("conv1"). 
	We will also consider 3 LAE-DNNs (see Section~\ref{s62}), and a regression task (Appendix~\ref{app:regr-task}).
...
\subsection{Comparison for global robustness with $L_\infty$ between {\em Diff}, VHAGaR and ITNE}

To be able to compare with VHAGaR, we use their benchmarks: 
VHAGaR bounds $\alpha_{C,D}$ 
computing the perturbation necessary to switch an image classified as class $C$ into an image classified as class $D$ (see \cite{vhagar} and appendix \ref{app.realtime}) instead of $\beta$ from section \ref{sec.realtime}, perturbation 
$L_\infty=0.05$, VHAGaR convolutional DNNs, for which the VHAGaR reduction rules of binary variables are implemented. 
We tested with 1000s and 14400s (4h) timeout. We report the upper bound $\bar{\alpha}$ each MILP model finds. For each DNN, we report the best lower bound $\underline{\alpha}$ among the MILP models, and report the gap between $\bar{\alpha}$ and  $\underline{\alpha}$ as $\%$ of increase over $\underline{\alpha}$. We report results in Table \ref{table.classical}. Recall that most robustness tools, e.g., $\alpha,\beta$-Crown, can only handle {\em local} robustness and cannot be tested.
...
\noindent {\bf Discussion:} This test is relatively easy, where the best upper bound $\bar{\alpha}$ is close to the lower bound $\underline{\alpha}$, with at most a $6\%$ gap. Using the {\em 1b} model is not useful, as its abstraction will lose more than $6\%$. 
%That is why it is not reported in Table \ref{table.classical}. 
VHAGaR computes upper bounds $\bar{\alpha}$ quite far from the lower bound, with a gap $40-120\%$. Although we are using its benchmarks, VHAGaR methodology is not well-suited for $L_\infty$ (it is more efficient on {\em Patch} \cite{vhagar}): ITNE is better, with gaps from $9-17\%$.
Our method provides the {\em Best} overall gaps: $0.6-6\%$, significantly tighter. Breaking up numbers, {\em 2b+1} is more accurate for shorter timeout, and {\em Diff} is more accurate when there is sufficient time, as expected. 
Anyway, both are better for particular classes $(C,D)$, as {\em Best} is lower than both {\em Diff} and {\em 2b+1} in every test.
...
\subsection{Comparison between LAE-DNNs and DNNs for $L_1$ perturbations.}
	
\label{s62}

We then turn to testing with $L_1$ perturbations and comparing between DNNs and their LAE-DNN versions (which are not supported in VHAGaR). Here, we consider MNIST-FC, which has a higher accuracy than MNIST-Conv, 
to test a different architecture than convolutional Networks.

LAE-DNNs are generated as described in Section 5: for each latent space dimension, we report in Table \ref{table.LAE} the accuracy loss from using LAE-DNNs vs using the Vanilla DNNs. 
Concerning the LAE algorithm, we experimented with PCA 
\citep{Paco}.
We set the dimension at the minimal dimension that results in at most 1$\%$ 
loss of accuracy, that is $20, 25$ and $60$ for  MNIST-FC, FMNIST-Conv and CIFAR10-Conv respectively, i.e. $>30$x dimension reduction in every case.


\begin{table}[h!]
	\centering
	\begin{tabular}{|c c c c c c c c c |}\hline
		latent space dimension: & 15 & 20 & 25 & 30 & 35 & 40 & ... & 60 
		
		\\\hline
		LAE-MNIST-FC  & 2$\%$ & {\color{blue} 0$\%$} & 0$\%$& 0$\%$ & 0$\%$& 0$\%$ & &0$\%$
		\\	
 		LAE-FMNIST-Conv & 5$\%$ &	3$\%$ &	{\color{blue} 1$\%$} & 0$\%$& 0$\%$	&0$\%$ & &	0$\%$ 
		\\
		LAE-CIFAR10-Conv  & 16$\%$ &	14$\%$ &	12$\%$ & 10$\%$ &	14$\%$ &	5$\%$ &	& {\color{blue} $1\%$}
		\\\hline
	\end{tabular}

	\caption{Loss of accuracy of LAE-DNN vs DNN according to the latent space dimension.
...
\begin{table}[b!]
	...
	\begin{tabular}{|cc| c | c c c|c|}\hline
		...
		\\\hline
		LAE-MNIST-FC  & 1000s & 2.52 & {\bf 2.43} & {\bf 2.43} & 2.68 & {\bf 2.41}
		\\
		...
		LAE-FMNIST-Conv & 1000s & 3.64 & 3.02 & 3.02 & {\bf 2.74} & {\bf 2.74}
		\\
		...
		\\\hline
		LAE-CIFAR10-Conv  & 1000s & 1.70 & 1.63 & 1.54 & {\bf 1.28} & {\bf 1.28}
		\\
	...
	\end{tabular}
	\caption{Comparison of upper bound $\overline{\beta}$ (lower is better $\downarrow$) averaged over all bounds $\overline{\beta}_{C,D}$,
	between ITNE and {\em Best} of {\em Diff, 2b+1 and 1b}, for DNNs and LAE-DNNs over MNIST, FMNIST and CIFAR10. %Best report is the average of the best for every case.
	 %We use latent space dimension = 20 for MNIST-FC, latent space dimension = 25 for FMNIST CNN1, and latent space dimension = 45 for CIFAR10 CNN1.
	 }
	\label{table.L1}
\end{table}
...
We consider the $\bar{\beta}$ bounds reached for $L_1$-perturbation  at most 1, on the 3 original DNNs MNIST-FC, FMNIST-Conv, and CIFAR10-Conv, plus their LAE variants for the dimension computed in Table~\ref{table.LAE}.
...
\noindent {\bf Discussion:} Compared with $L_\infty$ (Table~\ref{table.classical}), the gap between the upper bound $\bar{\beta}$ and 
the lower bound $\underline{\beta}$ is much larger with 
$L_1$ perturbations, from $1.5$x to $600$x. Using 
the faster {\em 1b} model now becomes pertinent. In half of the cases, {\em 1b} actually produces the best results. When the timeout is large enough compared to the complexity of the model, 2$b+$1 and sometimes {\em Diff} provide better bounds. ITNE is significantly worse, producing $25\%$ worse bounds 
$\bar{\beta}$ on average than {\em Best}.

LAE-DNNs reach much smaller upper bounds $\bar{\beta}$ than DNN: $15$x for MNIST-FC, $3$x for FMNIST-Conv, and $7$x for CIFAR10-Conv. 
%MNIST-FC is harder to solve than FMNIST-Conv, although they are comparable benchmarks, because fully connected architectures are richer than convolutional architectures of the same size. 
LAE-CIFAR10-Conv is the hardest test, because the latent space dimension (60) is not as small as for MNIST and FMNIST (20 and 25), which is due to the fact that the original CIFAR-10 dimensions (3072) is 4x larger than for MNIST (784).
Further, the {\em upper} bound $\bar{\beta}$ for LAE-CIFAR10-Conv (0.99) is much smaller than the {\em lower} bound $\underline{\beta}$ 
for CIFAR10-Conv (3.38), meaning that no other method 
would reach for CIFAR10-Conv a bound less than 3 times as high as the bound of our MILP models for LAE-CIFAR10-Conv. For FMNIST-Conv, it is 2x.
...
\subsection{Real-Time verification of Robustness from 
$(\bar{\beta}^\varepsilon_{C,D})_{C<D \leq 10}$}

\label{sec:realtime}


We now turn to how many images can be certified robust in real-time using the {\em Best} bounds $\beta^1_{C,D}$ computed in the previous subsection. We report the $\%$ of images certified robust in Table \ref{table.cert}, as well as the latency overhead to certify them: it is half a millisecond in every case (using a single CPU core), as it depends only on the number of outputs of the DNN, which is 10 for all 6 DNNs considered. It means 2000 images can be treated per second by each CPU core.
The complexity being linear with the number of classes, a dataset with 100 classes would need $\approx$ 5ms to be treated, 200 images/second per CPU core, which can be deemed {\em real-time}. 
We set a threshold of {\color{blue} $70\%$} to evaluate how large the $L_1$ perturbation can be while still certifying at least this threshold: a threshold much lower would mean labeling too many incoming images as uncertified, which would degrade the system too much.
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|cccc|c|}\hline
		...
		LAE-MNIST-FC  & {\color{blue} 80$\%$}  & 21$\%$ & 1$\%$ & 0$\%$ & 0.5 ms
		\\ \hline 
		
 		FMNIST-Conv & 
		{\color{blue} 75$\%$}  & 50$\%$ & 28$\%$ &	12$\%$  & 0.5 ms
		\\

		LAE-FMNIST-Conv & 
		{\color{blue} 92$\%$} &	{\color{blue}86$\%$} &	{\color{blue}79$\%$} & {\color{blue} 70$\%$}  & 0.5 ms
		\\\hline
		
		CIFAR10-Conv  & 39$\%$ & 0$\%$ & 0$\%$ & 0$\%$ & 0.5 ms
		\\ 
		LAE-CIFAR10-Conv  & {\color{blue}92$\%$} & {\color{blue} 72$\%$} & 61$\%$ & 51$\%$ & 0.5 ms
		\\\hline
	\end{tabular}

	\caption{Percentage of images certified robust in real-time 
	using the Best computed $(\beta^{1}_{C,D})_{C < D \leq 10}$, 
	for different values of $L_1$-perturbations.
	{\color{blue} Blue} depicts perturbation ensuring 
	{\color{blue} $\geq 70\%$} of images being robust. The latency overhead is the same as the number of classes (10) for all datasets.}
    %Latency overhead are the same as the number of classes is the same (=10) for all datasets.} 
	%2000 images/s can be treated per cpu core.}
    \label{table.cert}
\end{table}

With that, we can certify at least {\color{blue} $70\%$} of the images for all 3 LAE-DNNs for a $L_1$-perturbation of $1$, and even up to perturbation $2$ for CIFAR10 and even $4$ for FMNIST. We are unable to do it for the non-LAE version, but in the case of FMNIST-Conv, with a perturbation 4 times smaller than for its LAE-version. Overall, LAE-DNNs are amenable to certification in real-time, unlike vanilla DNNs.
