...
Our method provides the {\em Best} overall gaps: $.6-6\%$, significantly tighter. Breaking up numbers, {\em 2b+1} is more accurate for shorter timeout, and {\em Diff} is more accurate when there is sufficient time, as expected. 
...
would reach for CIFAR10-Conv a bound less than three times as high as the bound of our MILP models for LAE-CIFAR10-Conv. For FMNIST-Conv, it is 2 times.
...
We now turn to how many images can be certified robust in real-time using the {\em Best} bounds $\beta^1_{C,D}$ computed in the previous subsection. We report the $\%$ of images certified robust in Table \ref{table.cert}, as well as the latency overhead to certify them: it is half a millisecond in every case (using a single CPU core), as it depends only on the number of outputs of the DNN, which is 10 for all 6 DNNs considered. It means 2000 images can be treated per second by each CPU core, which can be deemed {\em real-time}.
The complexity being linear with the number of classes, a dataset with 100 classes would need $\approx$ 5ms to be treated, 200 images/second per CPU core. We set a threshold of {\color{blue} $70\%$} to evaluate how large the $L_1$ perturbation can be while still certifying at least this threshold: a threshold much lower would mean labeling too many incoming images as uncertified, which would degrade the system too much.
