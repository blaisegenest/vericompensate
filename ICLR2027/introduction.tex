...
for vanilla DNNs. While explicitly {\em characterizing} ID inputs to restrict the analysis to them is impractical, we propose an efficient alternative: Learn from ID samples an auto-encoder (AE), and then use the AE-DNN architecture (Fig.~\ref{fig:AE-DNN} in Section \ref{sec.pca}) to project the input on a smaller latent space. %, which aligns well with ID samples.
