...
Section~\ref{sec.pca}). 
The AE-DNN first uses an auto-encoder (AE) learnt from ID samples, so that the large input space is projected to a small latent space which aligns well with ID samples. It is then projected back to full space before being input to the original DNN. 
...
Both encodings are {\em asymptotically} exact (infinite runtime), but the {\em Diff} model is more accurate in practice (iso finite runtime) due to better linear relaxation.
...
on LAE-DNNs used in exploitation, for an $L_1$-perturbation of $1,4,2$ over the MNIST, Fashion-MNIST, and CIFAR-10 datasets, respectively, see Table \ref{table.cert}. The online process only adds  $\approx 0.5$ ms of latency per image, and $\approx 2000$ images/second can be treated per CPU core.
