...
\subsection{The classical encoding of ReLUs for global robustness}
	...
	However, $\mathcal{M}^{classical}$ only maintains $x_j$ and $x'_j$ separately, and both can take large values. This results into poor evaluation of $y_j$. This problem was first raised and experimentally demonstrated in \cite{diff}.
	...
	So even without explicit LP relaxation, the classical MILP encoding of \cite{MILP}, while asymptotically accurate, will result in looser bounds in practice. 
...
We now present our main methodological contribution, the {\em Diff} MILP encoding. It considers variables $x'_j,\hat{x}'_j, y_j, \hat{y}_j$, where $y_j, \hat{y}_j$ are the {\em Diff variables} \citep{diff}, with 
	...
	one binary variable $a'_j$, similar to ITNE \cite{ITNE}.
    However, unlike ITNE, 
...
We first display on Figure \ref{fig.2v} the function $f(x'_j,y_j)=\hat{y}_j$ for $y_j \in [- \gamma_j, \gamma_j]$. 
    ...
    Between these two linear planes, we have two more planes presenting intermediate cases, so 4 linear planes in total.
	

Recall first that $a'_j$ is the binary variable used in the 
classical MILP encoding for $\hat{x}'_j=\ReLU(x'_j)$.
Its value $a'=a'_j$ is settled by Prop.~\ref{Prop1},
reused in the MILP encoding of $\hat{y}_j = \ReLU(x_j)-\ReLU(x'_j)$:


\begin{proposition}
    \label{Prop2}
Assuming $y_j \in [-\gamma_j, \gamma_j]$ and $x'_j,y_j+x'_j \in [\alpha_j,\beta_j]$ with  $\alpha_j<0<\beta_j$, we have that $\hat{y}_j = \ReLU(x_j=y_j + x'_j) - \ReLU(x'_j)$ is the solution of the MILP program:
	\begin{align*}
		& \begin{aligned}
    ...
    $a'$ being shared with the classical MILP
    encoding for $\hat{x}'_j$.
\end{proposition}

To encode both $\hat{y}_j$ and $\hat{x}'_j$, the {\em Diff} encoding uses 2 binary variables, similar to the classical encoding.
...
This is a simplification (due to symmetry) of the additional linear constraint (3) of \cite{ITNE}.
...
Intuitively, the {\em diff values} will be small as they will be carrying the difference between both the image $I$ and its perturbation $I'$.
	...
	of \cite{MILP}, while fully accurate, will likely result in looser bounds as well. Notice that this was already witnessed in \cite{ITNE}, and some additional constraints were added to make the classical model more accurate: the {\em diff variables} were used as specific constraints added to the classical encoding (Eq. (3) in \cite{ITNE}). 
	%We will compare the different methods in Table \ref{table.classical}.
    ...
    \subsection{Abstractions of {\em Diff}: the {\em 2b+1} and {\em 1b} models}

\label{s.1b}


	We propose abstractions of the {\em Diff} encoding: 
	...
	The decoupling between $a'$ and $a'_j$ removes constraints in the MILP encoding, which helps the solver. However, the {\em 2b+1} model is not as accurate as the {\em Diff} model asymptotically.
...
\fi
    ...
    we introduce the efficient {\em 1b} model, which only uses the {\em diff} variables $y_j,\hat{y}_j$, and do not consider variables $x'_j,\hat{x}'_j$. 
...
the range of values $\hat{y}_j$ can take (see Figure \ref{fig.1v} in Appendix) corresponds to solutions of the following {\em 1b} MILP program with binary variable $a \in \{0,1\}$:
	\begin{align*}
