...
\label{s.notation}
	...
	To simplify the notation, we restrict the presentation to feed-forward, 
	...
	$\ReLU(x)=x$ for $x \geq 0$ and $\ReLU(x)=0$ for $x \leq 0$.
	An $\ell$-layer DNN is provided by $\ell$ weight matrices 
	$\boldsymbol{W}^i \in \mathbb{R}^{d_i\times d_{i-1}}$
	and $\ell$ bias vectors $\vb^i \in \mathbb{R}^{d_i}$, for $i=1, \ldots, \ell$.
	We call $d_i$ the number of neurons in the hidden layer 
	$i \in \{1, \ldots, \ell-1\}$,
...
\vspace{-0.4cm}
	...
	The vector $\hat{\boldsymbol{z}}$ is called post-activation values, $\boldsymbol{z}$ pre-activation values, 
	and $\boldsymbol{z}^{i}_j$ is used to denote the $j$-th neuron in the $i$-th layer. 
	...
	\end{align*} 
	In our practice, when we need global robustness, we will compute an optimization problem with respect to a certain number $\varepsilon$:	\begin{align}\label{global_robustness}
	...
	From the $\varepsilon$-diff bound, we cannot directly obtain a Lipschitz bound for the function, but we can get the following weaker bound:
	...
	\textbf{Mixed Integer Linear Programming (MILP)} can encode ReLU DNNs
faithfully	(extensions to other activation functions, e.g., maxpool in \cite{DivideAndSlide}):
	For an {\em unstable} neuron $n$, that is, with values 
    $x \in [\LB(n),\UB(n)]$ with constants $\LB(n)<0<\UB(n)$, the value $\hat{x}$ of $\ReLU(x)$ can be encoded exactly in an MILP formula \cite{MILP} with one binary/integer variable $a$ valued in $\{0,1\}$, using constraints:
...
\end{proposition}
	...
	%For all $x \in [\LB(n),\UB(n)] \setminus 0$, there exists a unique solution $(a,\hat{x})$ that meets these constraints, with $\hat{x}=\ReLU(x)$ \cite{MILP}. The value of $a$ is 0 if $x < 0$, and 1 if $x>0$, and can be either if $x=0$. This encoding approach can be applied to every (unstable) ReLU node, and optimizing its value can help get more accurate bounds. However, for networks with hundreds of {\em unstable} nodes, the resulting MILP formulation will contain numerous integer variables, and generally bounds obtained will not be accurate, even using powerful commercial solvers such as Gurobi.
	...
	MILP instances can be linearly relaxed into an LP over-abstraction, where variables $a$ originally restricted to integers in $\{0,1\}$ (binary) are relaxed to real numbers in the interval $[0,1]$, while maintaining the same encoding. Since solving LP instances takes polynomial time, this optimization is significantly more efficient. However, this efficiency comes at the cost of precision, often resulting in less stringent bounds \cite{DivideAndSlide}. This approach is termed the {\em LP relaxation}.
	...
	%To reduce the runtime, we will limit the size of the subset $X$. This a priori hurts accuracy. To recover some of this accuracy, we use an iterative approach: computing lower and upper bounds $\LB,\UB$ for neurons $n$ of each layer iteratively, which are used when computing values of the next layer.
	...
	First, SAS will call solvers to compute the LP model to get a solution, which is reasonably fast, as there are no binary variables. 
	...
	From paper CITE, we know that $\Utility$ is a safe overapproximation in the sense of the following proposition:
...
\paragraph{Classical MILP encoding for global robustness:}
In this paper, we consider the {\em global} verification problem, where we optimize over all images $I$ and perturbations $I'$ of $I$ with $|I-I'| \leq \varepsilon$. Consider:
    \begin{itemize}
    \item $x_j,\hat{x}_j$, for neurons $j$ with input image $I$; $x'_j,\hat{x}'_j$, for neurons $j$ with input the perturbed $I'$, 
    \item  $y_j = x_j - x'_j$ and 
	$\hat{y}_j = \hat{x}_j - \hat{x}'_j$ 
	the {\em diff variable} proposed by \cite{diff}. 
	%({\em not} $\hat{y}_j 	 = \ReLU(x_j-x'_j)$).
	%, 	similar to the Interleaving Twin-Network Encoding (ITNE) model of \cite{lipshitz,ITNE}. 
    \end{itemize}

	The classical encoding \cite{MILP} uses linear variables $x_j,\hat{x}_j, x'_j,\hat{x}'_j$, as well as integer variables $a_j$,
	resp. $a'_j$ for the encoding of $\hat{x}_j=\ReLU(x_j)$,
	resp. $\hat{x}'_j=\ReLU(x'_j)$, for all neuron $j$.
	Here are the {\bf set of constraints} $\mathcal{M}^{classical}$ to encode  global robustness with $L_\infty$-perturbations $ \leq \varepsilon$:
	\begin{itemize}
	   \item for all {\em input} neuron $j$ (in layer 0), $-\varepsilon \leq x_j-x'_j \leq \varepsilon$ \quad \, \quad  [$L_\infty$-perturbation $\leq \varepsilon$],
	   \item for all neuron $j$ from layer $i\geq 1$, 
	   $x_j = [\boldsymbol{W}^i\cdot \hat{\boldsymbol{z}}^{i-1}]_j+ \vb^i_j$ and 
	 $x'_j = [\boldsymbol{W}^i\cdot \hat{\boldsymbol{z}}'^{i-1}]_j+ \vb^i_j$,
	   \item for all neuron $j$, 
	   $\hat{x}_j$, resp. $\hat{x}'_j$, use Eq. (1) with integer variable $a_j$, resp. $a'_j$.
	\end{itemize}

\label{s.L1}

	\paragraph{Handling $L_1$:} To encode an $L_1$-perturbation, we use the following encoding {\em without} extra binary variables. 
	%Surprisingly, we could not find any reference to such an encoding in the literature.
	We use $k$ (= number of input neurons) additional {\em linear} variables 
	$(A_j)_{j \leq k}$, with constraints:
	\begin{enumerate}
...
\begin{proposition}
	\label{prop.L1}
	%\cite[Sec.~2.2.3]{mosek_cookbook}
	\cite{mosek_cookbook}
 Constraints 1 and 2 above are equivalent to $\sum_{j\leq k} |x_j-x'_j| \leq \varepsilon$.
\end{proposition}


%The proof or Prop. \ref{prop.L1} can be found in Appendix \ref{app.L1}. 
%It is crucial that 
The complexity of solving an MILP problem is bounded by $2^{bin} \times LP$ \cite{DivideAndSlide}, where $bin$ is the number of binary variables and $LP$ the P-time complexity of solving LP. 
No extra binary variables are used in the $L_1$ encoding.
The MILP objective function is defined in the next section.