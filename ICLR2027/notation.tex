...
\label{s.notation}
	...
	% which we extend componentwise on vectors.
...
\vspace{-0.4cm}
	...
	\end{align*} 
	In our practice, when we need global robustness, we will compute an optimization question with respect to a certain number $\varepsilon$:	\begin{align}\label{global_robustness}
...
\section{Classical MILP encoding for global robustness and L1-perturbations}

In this paper, we consider the {\em global} verification problem, where we optimize over all images $I$ and perturbations $I'$ of $I$ with $|I-I'| \leq \varepsilon$. We will consider:
    \begin{itemize}
	...
	The classical encoding \cite{vhagar} uses linear variables $x_j,\hat{x}_j, x'_j,\hat{x}'_j$, as well as integer variables $a_j$,
	resp. $a'_j$ for the encoding of $\hat{x}_j=\ReLU(x_j)$,
	resp. $\hat{x}'_j=\ReLU(x'_j)$, for all neuron $j$.
	Here are the {\bf set of constraints} $\mathcal{M}^{classical}$ to encode the global robustness problem with $L_\infty$-perturbations $ \leq \varepsilon$:
	\begin{itemize}
...
\label{s.L1}
	...
	To encode an $L_1$-perturbation, we use the following encoding {\em without} extra binary variables. 
...
\begin{proposition}
	\label{prop.L1}\cite[Sec.~2.2.3]{mosek_cookbook}
 Constraints 1 and 2 above are equivalent to $\sum_{j\leq k} |x_j-x'_j| \leq \varepsilon$.
\end{proposition}


The proof or Prop. \ref{prop.L1} can be found in Appendix 
\ref{app.L1}. 
It is crucial that no extra binary variables are used, as the complexity of solving an MILP problem is bounded by $2^{bin} \times LP$ \cite{DivideAndSlide}, where $bin$ is the number of binary variables and $LP$ is the P-time complexity of solving the LP problem. The objective function of the MILP program is defined in the next section.