...
We start by evaluating how accurate the 2v model is vs the classical encoding vs the interleaving twin-network encoding (ITNE) from \cite{lipshitz}, that we reimplement in Gurobi by considering the classical encoding plus the explicit constraints $\frac{x_i-x_i' - \gamma}{2} \leq \hat{x}_i-\hat{x}_i' \leq \frac{x_i-x_i' + \gamma}{2}$, corresponding to Eq. (3) of \cite{lipshitz} (and also to the relaxation of our "1v" model). We compare when half the ReLUs are approximated with the LP relaxation, and when all the ReLUs are encoded exactly. We use the pipe models which is easier to verify, and set the time-out at 1000s for all models.
...
\begin{table}[h!]
    ...
    \\\hline
	ITNE, $100 \times 2$ &    $.045$ &  $.023$ & {\bf .023} 
    \\\hline
...
in Table \ref{table.mnist} one particular bound $\beta^{.5}_{i,j}$, namely for $i=6,j=8$ as confusion between 6 and 8 happens.
	...
	\begin{tabular}{||l||c|c|c||}\hline\hline
		model &        Bound $\downarrow$ &  Sol. &      Worst-Case $\uparrow$ \\\hline \hline
		1v, $500 \times 1$ & $15.67$ & $.620$ & $.012$ \\\hline 
		3v, $500 \times 3$ & $16.09$ & $.755$ & {\bf .207} \\\hline 
	    2v, $500 \times 2$ & {\bf 8.99} & nan & nan \\\hline\hline	 
	\end{tabular}
	\caption{Comparison of "1v", "3v" and "2v" models 
	to obtain bounds on $\beta^{.5}_{6,8}$ on the {\bf full dimension} MNIST DNN, 
...
with maximal $\beta^{.5}_{6,8}=0.207$ for MNIST as obtained by the "3v" $500 \times 2$ model in {\bf full 784 dimension} image space.}
\label{fig3}
\end{figure*}	
...
\caption{An image and its perturbation from the {\bf 20-dimension reduced space} with maximal $\beta^{.5}_{6,8}=.084$ for MNIST as obtained by the "2v" $500 \times 2$ model.}
\label{fig4}
\end{figure*}	
...
2v, $500 \times 2$ &  $1.182$ & $.084$& {\bf .084}  \\\hline\hline
	...
	\end{tabular}
	\caption{Comparison of "1v", "3v" and "2v" models 
	to obtain bounds on $\beta^{.5}_{6,8}$ on the {\bf 20 dimension}  reduced order MNIST DNN, for timeouts of 14400s, where 
	all the %400, 475,  or 
...
\begin{table}[b!]
	...
	\end{tabular}
	\caption{Comparison of "1v", "3v" and "2v" models 
	in the percentage of images certified robust in real-time 
	using the computed $(\beta^{.5}_{i,j})_{i < j \leq 10}$  for different values of $L_1$-perturbations.}
    \label{table.cert}
\end{table}
...
as in Table \ref{table.mnist}, obtained with the same time-out. The bounds are directly comparable: the best $\beta^{.5}_{6,8}$ obtained using the reduced dimension is $\approx 8$ times smaller than when using the full dimension ($1.137$ vs $8.99$). With these bounds, most images ($86\%$) can be certified in real-time robust for a perturbation $L_1 \leq .5$, and even $53\%$ with a perturbation $L_1 \leq 1$ twice as large, see Table \ref{table.cert}. We illustrate that improbable images are removed by displaying in Fig.~\ref{fig4} the worst-case obtained for $\beta^{.5}_{6,8}$, which indeed looks like a realistic MNIST instance.
