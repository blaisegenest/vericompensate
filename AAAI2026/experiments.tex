	...
	for an $L_1$-deformation $\leq .5$. The pipe considers a mixture of $L_1 \leq 3.9$ and $L_\infty \leq 0.2$ perturbations, corresponding to physically pertinent dimensions, and 
...
the worst-case will equal the solution, otherwise, it will be smaller.

\subsection{Classical vs our "2v" model}

We start by evaluating how accurate the 2v model is vs the classical encoding, both when half the ReLUs are approximated with the LP relaxation, and when all the ReLUs are encoded exactly. We use the pipe models which is easier to handle, and set the time-out at 1000s for both models.
...
We confirm both that the LP relaxation of the classical model is extremely poor, with bounds 8 times worst than using our "2v" model. Further, even using a fully accurate mode with all 100x2 variables encoded as binary variables, the "2v" model produces bounds $>4$ times better than the classical model, due to the internal Branch and Bound process to compute bounds, which uses linear relaxation.
