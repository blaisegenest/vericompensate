...
In this paper, we consider {\em global} robustness, that is guarantees on robustness not restricted to a set of local images.
...
using partial MILP (pMILP) methodology. 
We propose several such pMILP models for global robustness, with different trade-offs.
...
This results into usable bounds, allowing in real time to certify robustness for $87\%$ of incoming images in the MNIST benchmark for a L1-perturbation of $\frac{1}{2}$, as well as for a surrogate computing the hidden plastic strain 
