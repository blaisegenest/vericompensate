...
Similar to the second model, the constraints on $x_i$ for $\hat{x}_i=\ReLU(x_i)$ are not necessary, or at least can be relaxed without any loss in accuracy.
	...
	\begin{tabular}{|l|c|c|c|c|}\hline
		 $L_1 \leq 0.5$ & Bound & Solution & Real & Time \\\hline
	...
	\end{tabular}
	\caption{Comparison of "1v", "3v" and "2v" models on the full dimension MNIST DNN. with a fixed timeout of 1000s, 14523s and 72126s, where 90 ($\times 2$, $\times 3$) or all 100 ($\times 1, \times 2,\times 3$) neurons use binary variables.}
\end{table}
