...
The exponential complexity with the number of nodes is obvious, as shown on Figure \ref{fig3}, where time is represented using logarithmic scale. 
Notice that when certifying, pMILP uses $|X| \in$ 21-24, which is is a good trade off between time and accuracy.
...
\begin{figure}[h!]
	...
	USE STRAIGHT LINES. DO NOT WRITE A TITLE OF GRAPH IN THE PNG (plot of method). CHANGE THE AWFUL LEGEND TO "pMILP". GRAPHS HAS COUPLE OF PROBLEMATIC POINTS LOWER than previous  (TYPOS?)
	ADD LOGSCALE AFTER "(seconds, logscale)". USE SAME COLOR FOR pMILP THAN IN FIGURE 4}
	\label{fig3}
\end{figure}
...
\begin{figure}[h]\hspace*{-0.8cm}
	...
	USE THE EXACT SAME REPRESENTATION THAN IN FIGURE 3 (e.g. time (second, logscale), no title of graph, time is Y axis}
	\label{fig4}
\end{figure}
