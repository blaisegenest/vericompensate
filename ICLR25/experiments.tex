...
Formally, the uncertainty of a node $a$ with bounds $[\LB,\UB]$ is $unc(a) = \UB - \LB$. The average uncertainty of layer $\ell$ is $\dfrac{\sum_{a\in l} unc(a)}{size(\ell)}.$
...
We focus on the uncertainty of nodes in the third layers, wrt ReLU nodes in the first and second layer. The bounds for nodes of the first two layers are computed exactly using the full MILP encoding. We report in Table \ref{tab:example1} the average uncertainty of $\MILP_X$ following the choice of the $K$ heaviest neurons for our {\bf Utility} function, compared with a random choice, both for nodes exclusively from the previous Layer 2 or from both Layers 1 and 2. We compared with choosing based on strength$(n) = (\UB(n)-\LB(n))\cdot |W_{cd}|$ \cite{DivideAndSlide}, which is limited to the previous Layer 2.

\begin{table}[h!]	
	\centering
	\begin{tabular}{|c||c|c|c||c|c|}
		\hline
			&\multicolumn{3}{c||}{$X \subseteq$ Layer 2, max $=61$}&\multicolumn{2}{c|}{$X \subseteq$ Layers 1\&2, max $=116$}\\\cline{2-6}
		\text{$|X|$}  & \text{Random} & \cite{ DivideAndSlide} & \bf Utility& \text{Random} &\bf Utility\\ \hline
		\hline
		0  (LP) &   1.761  &1.761& 1.761 & 1.761  & 1.761\\ \hline \hline
		5  &   1.729&1.704 & 1.603&  1.729&  \bf 1.532 \\ \hline
		10  &  1.701 &  1.651 &1.517& 1.696 & \bf  1.371\\ \hline
		15  & 1.671 &  1.599 &1.466&  1.653& \bf  1.247\\ \hline
		20  &  1.635 & 1.557&1.438 & 1.619 & \bf  1.145\\ \hline
		25  &  1.601 & 1.519 &1.427&  1.586 & \bf 1.061\\ \hline
		30  & 1.574 & 1.489 &1.425& 1.546 & \bf  0.989 \\ \hline
		35  &  1.542 & 1.465&1.424 & 1.502 & \bf 0.934 \\ \hline
		40  & 1.512 & 1.447 &1.424& 1.469 & \bf 0.921 \\ \hline \hline
		max & 1.424  &1.424& 1.424 & 0.895 & 0.895   \\ \hline
	\end{tabular}
...
Namely, these are 6 {\em hard} ReLU-DNNs: 5 MNIST DNN from the ERAN GitHub (\url{https://github.com/eth-sri/eran}, the 4th to the 8th DNNs provided) as well as 1 CIFAR CNN from 
\cite{AdversarialTrainingAndProvableDefenses} (the network file we use is from \url{https://github.com/Verified-Intelligence/alpha-beta-CROWN}), also see \cite{SDPFI}, with the same $\epsilon$ setting. For reference, we also report an easy but very large ResNet Network for CIFAR10, already tested with $\alpha,\beta$ CROWN. We report in Table \ref{table_hybrid} the $\%$ of undecided images, that is the $\%$ of images than can be neither falsified (by $\alpha,\beta$-CROWN) nor verified by the tested verifier, among the 100 first images for each MNIST or CIFAR10 benchmark, the exact same setting and DNNs as used in Table \ref{table_beta}.
...
For that, we considered the DNN 6x500, and the 20 first images from the MNIST benchmark.
...
{\em Analysis}: in 90\% of the cases, Hybrid MILP is very close to the upper bound. On 2 images (10 and 13), Hybrid MILP is far from the upper bound: these are also the ones where the upper bound is the highest, so it is possible the upper bound did not find a close attack to robustness.
...
We volontarily limited the comparison to $\alpha,\beta$-Crown because it is the most efficient verifier available to date, and it enabled us to consider much finer parameter choice without too much cluter. 
