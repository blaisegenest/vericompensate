...
Last, compared with refined $\beta$-Crown, we can observe three patterns: on the shallowest DNNs (5x100, 5x200), refined $\beta$-Crown can run full MILP on almost all nodes, reaching almost the same accuracy than our Hybrid MILP, but with longer runtime (up to 2x on 5x100). On intermediate DNNs (8x100,8x200), full MILP invoked by refined $\beta$-Crown can only be run on a fraction of the neurons, and the accuracy is not as good as Hybrid MILP, with $6\%-8\%$ more undecided images, while having longer runtime. Last, refined $\beta$-Crown cannot scale to larger instances (6x500, CNN-B-Adv), while Hybrid MILP can.
...
Notice that several other verifiers were already reported \cite{crown} on these same DNNs, with defavorable comparison towards $\alpha,\beta$-CROWN (PRIMA \cite{prima}, SDP-FO \cite{SDPFI}, etc). Further, we reported accuracy of complete verifiers, NNenum \cite{nnenum}, Marabou \cite{Marabou, Marabou2}, respectively 4th, 3rd 
...
Last, Pyrat code is not open source, which made running it impossible.
