...
In our algorithm, we will open nodes at most 3 layer3 before the target layer. So the formula will consists of three parts.
...
Suppose $a$ is a node in one layer before $z$. To estimate Improve\_max$_a^z$:
...
\begin{definition}
	...
	%In particular, if $v(\hat{a})=\ReLU(v(a))$, then we will have 
	%\begin{align*}
%		Utility^z(a) = 0.
%	\end{align*}
