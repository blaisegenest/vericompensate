...
\begin{document}
	...
	\end{align*} 
	In our practice of global robustness, we will consider the following weaker property with respect to a certain number $\varepsilon$:
	\begin{align*}
		\forall \boldsymbol{x} \forall\boldsymbol{y} &  (|\boldsymbol{x}-\boldsymbol{y}| \geq \varepsilon \implies|f(\boldsymbol{x}) -f(\boldsymbol{y}) |\leq K|\boldsymbol{x}-\boldsymbol{y}|\\
		&  |\boldsymbol{x}-\boldsymbol{y}| < \varepsilon \implies|f(\boldsymbol{x}) -f(\boldsymbol{y}) |\leq K\varepsilon)
	\end{align*} 
	In terms of the code, it is compute an optimization question respect to a certain number $\varepsilon$:	\begin{align}\label{global_robustness}
	...
	\begin{enumerate}
		\item Add constraints for input nodes between $\mathcal{M},\mathcal{M}'$ for the requirement $|\boldsymbol{x}-\boldsymbol{y}| \leq \varepsilon$.
