...
\end{enumerate}
	...
	We can improve the accuracy of the simplified model by simply changing the constraints, which is a trade-off between accuracy and speed.
...
%	Moreover, we can add two more natural constraints: $x_i'+y_i \geq \alpha_i \hspace*{1ex}\wedge\hspace*{1ex}  x_i'+y_i \leq \beta_i.$
	...
	The accurate constraints for $\hat{y}_i = \ReLU(x'_i+y_i)-\ReLU(x'_i)$ are the follows: \begin{align*}
	...
	\end{align*}Here, $a,a'$ are the binary variables, and $a'$ also play the role binary for $\hat{x}'_i=\ReLU(x'_i)$.
