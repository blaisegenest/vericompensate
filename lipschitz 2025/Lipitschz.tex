...
However, the problem is that even relaxing a few nodes can cause a significant loss of accuracy, which leads us to find another modeling approaches.
...
\end{enumerate}
	...
	The key point is that, we can relax all other binary variables for $\hat{x}_i = \ReLU(x_i)$ and $\hat{x}'_i = \ReLU(x'_i)$.
	Then the simplified model is that it only contains the number of binary variables as MILP model for local robustness, hence the running speed of this is much faster comparing to the previous one. The cost is a loss of accuracy when the time is infinite. 
	...
	We can improve the accuracy of the simplified model by simply changing the constraints, which is a trade-off between accuracy and speed. (This model has the same binary set, although the meaning of binary variable for $y_i$ is somehow different.)
...
%	Moreover, we can add two more natural constraints: $x_i'+y_i \geq \alpha_i \hspace*{1ex}\wedge\hspace*{1ex}  x_i'+y_i \leq \beta_i.$
	...
	the accurate constraints for $\hat{y}_i = \ReLU(x'_i+y_i)-\ReLU(x'_i)$ (together with $\hat{x}'_i=\ReLU(x'_i)$) are the follows:
	\begin{align*}
		& \begin{aligned}
		...
		\end{aligned}
	\end{align*} Here, $a,a'$ are the binary variables, and $a'$ also play the role of binary variable for $\hat{x}'_i=\ReLU(x'_i)$. The constraints for $x_i$s are not necessary, or at least their MILP constraints can be relaxed to LP without reducing accuracy.
...
%	\end{align*} 
	...
	A variant of the above model is to distinguish the binary variables $a'$s in the above constraints and the binary variables for $\hat{x}'_i=\ReLU(x'_i)$, and relax all the latter ones.
	...
	We will do experiments on two networks for MNIST dataset and another physical model.
	
	Among two MNIST $5\times 100$  full-connected DNN, one is a normally trained neural network and another is for trained for adversarial robustness. We call them MNIST $5\times100$-normal and MNIST $5\times 100$-DiffAI. They have the same structure: 5 hidden layer with 100 neurons each, 10 outputs and 784 inputs.
	
	
	The physical network is a simplified model with 10 input neurons, 26 output neurons, and 2 hidden layers with 50 neurons each.
