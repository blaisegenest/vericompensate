...
\begin{enumerate}
\item {\em Training} the PCA-encoding and decoding on the training dataset (e.g., MNIST, FMNIST, CIFAR10) \eca{to define the PCA-representation}. Note that the original DNN has been previously trained on this training dataset.
...
%
In contrast, when using the PCA-DNN, the image and perturbed image in Fig.~\ref{fig4} are much more natural. This occurs because the output of the PCA-encoding/decoding (from reduced space) is its own image\todo{the same image? or same dimension? img is like slightly changed}, and thus can be used as original input, producing the maximal $\underline{\beta}_{6,8}=.084$.
