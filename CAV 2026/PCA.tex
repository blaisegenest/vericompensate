...
\eca{To address this, we leverage principal component analysis (PCA) \cite{Paco}, a technique from engineering science, to restrict the output (hence lowering the upper bounds) {\em without} restricting the input. Specifically, PCA extracts the most important linear components of a dataset by computing linear operators to project data from the full input dimension to a reduced feature space (encoding) and back (decoding). This reduced feature space retains the most important features of the full-dimensional space. This enables it to represent faithfully common ({\em in-distribution (ID)}) inputs, while representing OOD inputs unfaithfully. In addition, observe that, with PCA, the worst cases are more "natural" (ID) and representation of the actual dataset (see Fig.~\ref{fig4}). Since DDNs are inherently inaccurate on OOD inputs, misrepresenting them does not make DNNs any worse. To ensure that ID inputs are faithfully represented, we verify that the PCA representation does not lower the DNN accuracy on these inputs.}
