...
We construct the {\em PCA-DNN} by prepending \eca{PCA-encoding then decoding} to the original DNN, transforming an image into its {\em PCA-representation}, see Fig.~\ref{fig.MNIST}.
%This reduced feature space retains the most important features of the dataset. 
The PCA-representation is faithful for common {\em in-distribution (ID)} inputs (e.g., Fig.~\ref{fig4}), though it may become unfaithful for OOD inputs. Since DNNs are inherently unreliable on OOD inputs, misrepresenting them does not impact accuracy, \eca{which we check empirically}.
