...
\eca{Global upper bounds $\bar{\beta}$ obtained across the entire input space tend to be overly pessimistic, since they must accommodate all possible points, including out of distribution (OOD) \cite{OOD} samples far from the training dataset. Thus, we propose to leverage principal component analysis (PCA) \cite{Paco}, a technique from engineering science, to restrict the verification objective to common, i.e., in-distribution (ID), data. 
%
While OOD inputs may be represented unfaithfully into this ID subspace, this is justified by the DNN's inherent lack of robustness outside the training distribution. 
To ensure that a dataset is faithfully represented, we verify the DNN's accuracy on the representation of this ID dataset \todo{on the representation of this ID dataset? what?}. In addition, we can observe that examples corresponding to the lower bounds $\underline{\beta}$ with PCA are more "natural" (ID) wrt the dataset (compare Fig.~\ref{fig3} and Fig.~\ref{fig4}).}
...
We propose to leverage Principal Component Analysis (PCA) \cite{Paco},
%Model Order Reduction, 
stemming from Engineering Science, to focus on faithfully representing common inputs from the dataset ({\em ID}: in distribution). OOD inputs may be represented unfaithfully (with (ID) representation closer to the dataset), which is reasonable as the DNN is unlikely to provide reasonable answers on OOD inputs anyway. 
To make sure that a dataset is faithfully represented, we can check the accuracy of the DNN on the representation of this ID dataset.
Last, examples corresponding to the lower bounds $\underline{\beta}$ with PCA will be much more natural (ID) wrt the dataset (compare Fig.~\ref{fig3} and Fig.~\ref{fig4}).
\fi

%The simplest Model Order Reduction uses 
 PCA is a %Model Order Reduction 
 technique to extract %the main information from a dataset.
%reduce the dimension of a dataset while keeping its main information. 
%From a dataset, 
the most important linear components from a dataset. 
%are computed. Projecting over the first few main components is a powerful model order reduction technique ({\em encoding} / {\em reducing from the full dimension}). 
It computes linear operators to go from the full dimension to a reduced dimension of important features (encoding), and back (decoding).
%It is easy to go from the reduced dimension back to the full dimension
%by making the product between the reduced vector and the first few dimensions ({\em decoding}). 
The PCA-pipeline is as follows (see Fig.~\ref{fig.MNIST} for an example on the MNIST dataset):

\begin{figure*}[b!]
	\centering
\includegraphics[scale=0.4]{image.png} \hspace{1.5cm}
\includegraphics[scale=0.4]{perturb.png}
\caption{An (OOD) image for MNIST and its perturbation ($L1$-difference of $.5$ in a unique pixel at x=14, y=8 from top) maximizing
...
\begin{enumerate}
\item {\em Training} the PCA encoding and decoding from the Training Dataset (e.g., MNIST, FMNIST, CIFAR10). Notice that the original DNN has been previously trained on this training dataset.
...
We check the accuracy of the PCA-DNN, that is, the $\%$ of images where the class predicted matches the ground truth. We set the PCA dimension as small as possible (for more efficiency) such that the loss of accuracy of PCA-DNN is minimal ($ \leq 1 \%$) wrt the original DNN without PCA (20 for MNIST in Fig.~\ref{fig.MNIST}).
% enccode/decode.
...
%On the other hand, 
Last, the input of PCA-DNN is not restricted, and results from real-time verification hold for any arbitrary image, even OOD images.
...
\caption{An (ID) image for MNIST and its perturbation when using the {\bf PCA-DNN} with 20 dimensions, maximizing $\underline{\beta}^{.5}_{6,8} = .084$, as obtained by the{\em Diff} model.}
\vspace{-0.2cm}
\label{fig4}
\end{figure*}	
