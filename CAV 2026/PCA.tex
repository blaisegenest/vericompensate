...
We propose to leverage on Principal Component Analysis (PCA) \cite{Paco},
...
Last, examples corresponding to 
%This will have several benefits: the search is faster, it produces better, and 
the lower bounds $\underline{\beta}$ with PCA will be much more natural (ID) wrt the dataset (compare Fig.~\ref{fig3} and Fig.~\ref{fig4}).
...
\caption{An (ID) image for MNIST and its perturbation when using the {\bf PCA-DNN} with 20-dimension, maximizing $\underline{\beta}^{.5}_{6,8} = .084$, as obtained by the{\em Diff} model.}
\vspace{-0.2cm}
\label{fig4}
\end{figure*}	
