...
\item Experimentally, the exact {\em diff MILP model} computes upper bounds 
which are 1 order of magnitude better than with the classical MILP encoding, and much closer (Table \ref{table.classical}) to the lower bounds than with variants of the classical encoding; namely reducing the number of binary variables in a complete fashion (Vhagar \cite{vhagar}); or adding linear relaxation for the diff variables (ITNE \cite{ITNE}).
...
Further, using PCA reduces $\beta_{C,D}$ by up to an order of magnitude.
Overall, the "1b+2" model using PCA enables the robustness certification of $69\%$ of MNIST images for an $L_1$-perturbation of $1$, resp. $89\%$ for $.5$. The online process adds only 0.0005s of latency per image, and 2000 images/second can be treated per CPU core.
\end{enumerate}
