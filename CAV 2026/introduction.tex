...
\begin{enumerate}
	...
	\cite{vhagar,lipshitz,ITNE}, which considers the input and the perturbation but not the diff variable, we keep the same number of 2 binary variables per ReLU.
	...
	we develop two abstract MILP models, which are more efficient but also asymptotically less accurate. Namely, the "2b+1" model, decoupling one binary variable into 2 variables: 1 binary and the other using linear relaxation; while the "1b" model has a unique binary variable per ReLU, only considering the {\em diff variables}.
   ...
   %Check \cite{L1paper-by-AI} ??
	...
	To address this, we consider model order reduction techniques from engineering science \cite{Paco}. Specifically, we use Principal Component Analysis (PCA) to represent faithfully common inputs from the dataset.
	OOD	inputs may be represented unfaithfully, which is reasonable as the DNN is unlikely to provide reasonable answer on such inputs anyway.
...
$\bar{\beta}^{\varepsilon}_{C,D}$ which reduces the gap to the lower bound by one order of magnitude compared with optimized version of the classical MILP encoding; namely reducing the number of binary variables (Vhagar \cite{vhagar}); or adding linear constraints from the diff variables (ITNE \cite{ITNE}), see Table \ref{table.classical}.
...
Further, using PCA reduces the upper bound $\beta_{C,D}$ by $2$ to $17$ times.
Overall, using this different contributions (diff MILP model, abstraction and PCA), it enables the real-time certification of $\geq 69\%$ of fresh images for an $L1$-perturbation of $1$ over three datasets (MNIST, Fashion MNIST and CIFAR-10), see Table \ref{table.cert}. The online process adds only 0.5ms of latency per image, and 2000 images/second can be treated per CPU core.
\end{enumerate}
