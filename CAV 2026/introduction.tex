...
%
This check typically requires only a few dozen CPU instructions, which can be completed under 1 ms on a single CPU core. \eca{When an image fails this certification, one could either skip it (e.g., in a video stream) or revert to a safer, degraded mode until a trustworthy, robust image is received.}
