...
$L_\infty$-perturbation, i.e. every subpixel of the input image can vary in a very small range, typically $\frac{2}{255}$ (that is 2 levels of grey/blue/right/green). 
...
output values of different decision classes due to the perturbation, close to \cite{vhagar}. That is, considering decision classes $C$ and $D$ and perturbation $\varepsilon$, compute 
...
\begin{enumerate}
	...
	where the variables are the values of the perturbed neurons, as well as the difference between the original and the perturbed neuron values (called the {\em diff variables}, introduced in \cite{diff}). We study and encode how the {\em diff variables} evolve after passing through a ReLU (Prop.~\ref{Prop2}). Compared with the {\em classical MILP model} \cite{MILP} employed in \cite{vhagar,lipshitz,ITNE}, which considers the input and the perturbation but not their difference ({\em diff variable}), we keep the same number of 2 binary variables per ReLU. Experimentally however, 
	...
	%Experimentally, 
	our {\em diff MILP model} provides more accurate bounds at a given runtime
	...
	we develop two abstract MILP models, which are more efficient but also possibly less accurate.
	Namely, the "1b+2" model replaces the 2 binary variables by 1 binary (for the diff variable) and 2 linear variables (linear relaxation for the perturbation); while the "1b" model has a unique binary variable per ReLU, only considering the {\em diff variables}. 
   ...
   \item  In terms of perturbations, we consider conjunctions of $L_\infty$- and $L_1$-norms, which allow to accurately describe perturbations. For instance, "each subpixel is perturbed by at most $\frac{50}{255}$ ($L_\infty$) and the sum of the absolute value of perturbations over all subpixels is at most $1$" ($L_1$-perturbation). While $L_1$-perturbations are not linear (because of the absolute values), reason for which it is seldom used, Prop.~\ref{prop.l1} shows how to use it as a perturbation in the MILP model without incurring any expansive binary variables (only cheap linear variables are necessary). Check \cite{L1paper-by-AI} ??
	...
	To address that, we consider model order reduction techniques from engineering science \cite{Paco}. Specifically, we use Principal Component Analysis (PCA) to represent faithfully realistic inputs.
	OOD	inputs may be represented unfaithfully - but the DNN is unlikely to provide reasonable answer on such inputs anyway.
	...
	For instance, using just $20$ out of 784 dimensions suffices to represent faithfully MNIST inputs, without losing accuracy for the DNN on MNIST inputs.
...
\item Experimentally, the exact {\em diff MILP model} computes bounds 
$\beta_{C,D}$ which are 1 order of magnitude better than with the classical MILP encoding, and much closer (Table \ref{table.classical}) to the lower bound than with variants of the classical encoding; namely Vhagar \cite{vhagar} which reduces the number of binary variables in a complete fashion; and ITNE \cite{ITNE} which adds linear relaxation for the diff variables.
Our "1b+2" variant offers good trade-offs, reaching better bounds in the same runtime as the diff MILP model. The "1b" model converges faster but bounds are looser, interesting when runtime is very limited or problem is very complex (Tables \ref{table.mnist},\ref{table.pipe}).
Overall, the "1b+2" model obtains bounds $\beta_{C,D}$ which enables the robustness certification of $69\%$ of MNIST images for an $L_1$-perturbation of $1$, resp. $89\%$ for $.5$. The online process adds only
0.0005s of latency per image, and 2000 images/second can be treated per CPU core.
\end{enumerate}
