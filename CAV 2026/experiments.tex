...
bound $\alpha$ they consider, perturbation they consider (here $L_\infty=0.05$) on their convolutional DNNs, for which the reduction rules of binary variables are implemented. We tested with 1000s and 14400s (4h) Time-Out. For each DNN and MILP models, we report the upper bound $\bar{\alpha}$ they find. For each DNN, we report the best lower bound LB $\underline{\alpha}$ found among all the MILP model, and report the gap between the upper bound $\bar{\alpha}$ and the lower bound $\underline{\alpha}$ as $\%$ of increase over $\underline{\alpha}$. We report results in Table \ref{table.classical}
