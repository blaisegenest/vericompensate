	...
	We consider 4 pretrained DNNs on 3 different dataset (MNIST, Fashion MNIST and CIFAR10). Namely,  MNIST-FC, a fully connected DNN available at \url{https://github.com/eth-sri/eran} ("5x100"). We also considered 3 convolutional DNNs: MNIST-Conv, FMNIST-Conv, and CIFAR10-Conv, 
	from \cite{vhagar} ("MNIST-C1, FMNIST-C1, and CIFAR10-C1").
	We will also consider 3 PCA-DNNs, as detailled in Section~\ref{s62}
...
\subsection{Comparison with Vhagar and ITNE ($L_\infty=0.05$)}

To be able to compare with Vhagar, we have to use Vhagar benchmarks. We use
bound $\alpha$ they consider, perturbation they consider (here $L_\infty=0.05$) on their DNNs (here Conv for MNIST, FMNIST and CIFAR-10), for which the reduction rules of binary variables are implemented. We tested with 1000s and 14400s (4h) Time-Out. For each DNN and MILP models, we report the upper bound $\bar{\alpha}$ they find. For each DNN, we report the best lower bound LB $\underline{\alpha}$ found among all the MILP model, and report the gap between 
the upper bound $\bar{\alpha}$ and the lower bound $\underline{\alpha}$ as 
$\%$ of increase over $\underline{\alpha}$. We report results in Table \ref{table.classical}
...
\noindent {\bf Discussion:} This test is relatively easy, where the best upper bound $\bar{\alpha}$ found is close to the lower bound $\underline{\alpha}$ found, with at most a $6\%$ gap. For that reason, using the 1$b$ model is not useful, as its abstraction will loose more than $6\%$. That is why it is not reported in Table \ref{table.classical}. Vhagar computes upper bounds $\bar{\alpha}$ quite far from the lower bound, with gap at least $40\%$, up to $120\%$. Although we are using Vhagar benchmarks and CNNs, Vhagar methodology is not well-suited for $L_\infty$ (it is more efficient on {\em patch} \cite{vhagar}). ITNE is better, with gaps from $9\%$ to $17\%$.
Our methods based on the {\em Diff} MILP provides the {\em Best} overall gaps, from as low as $.6\%$ to $6\%$, significantly tighter. Breaking up numbers, 2$b$+1 is more accurate overall for shorter timeout, and {\em Diff} is more accurate when there is sufficient time, which is to be expected. Anyway, both are better for particular classes $(i,j)$, as {\em Best} is lower than both {\em Diff} and 2$b$+1 in every test.
...
Now, we consider the $\bar{\beta}$ bounds reached for $L_1$-perturbation  at most 1, on the 3 original DNNs MNIST-FC, FMNIST-Conv and CIFAR10-Conv, plus their PCA variants for the dimension computed in Table~\ref{table.pca}.
...
\begin{table}[b!]
	...
	\begin{tabular}{|cc| c | c c c|c|}\hline
		...
		\\
		LB $\underline{\beta}$= 0.0015 & 14400s & 1.69 & 1.56 & 1.52 & {\bf 1.15} & {\bf 1.15}
		\\
		 extra long timeout& 216000s & & & 1.36 & {\bf 0.99} & {\bf 0.99}
		
		\\\hline

	\end{tabular}
	\caption{Comparison of upper bound $\overline{\beta}$ (lower is better $\downarrow$) averaged over all bounds $\overline{\beta}_{i,j}$,
	between {\em Diff, 2b+1 and 1b}, for both original DNN as well as PCA-DNNs over MNIST, FMNIST and CIFAR dataset. %Best report is the average of the best for every case.
...
\noindent {\bf Discussion:} Compared with results on $L_\infty$ (Table~\ref{table.classical}), the upper bound $\bar{\beta}$ computed by Best is now much further from the lower bound $\underline{\beta}$: from 2 to 600 times. Using 
the faster 1$b$ model now becomes pertinent. In half of the cases, 1$b$ actually produced the best results. When the timeout is large enough compared to the complexity of the model, 2$b+$1 and sometimes {\em Diff} provide better bounds. ITNE is significantly worse, producing results on average $25\%$ worse boundS $\bar{\beta}$ than Best.

Comparing with and without PCA, as expected, PCA-DNNs allow to reach much smaller upper bounds $\bar{beta}$ than without PCA, $15$x for MNIST-FC, $3$x for FMNIST-Conv and $7$x for CIFAR10-Conv. MNIST-FC is harder to solver than FMNIST-Conv, although they are comparable benchmark, because fully connected architecture are richer than Convolutional architecture of the same size. PCA-CIFAR10-Conv is the hardest test, because the dimension (60) is not as reduced (20,25) for MNIST and FMNIST.
...
We now turn to how many images can be certified robust in real-time using the {\em Best} bounds $\beta^1_{i,j}$ computed in the previous subsection. We report the $\%$ of images certified robust in Table \ref{table.cert}, as well as the latency overhead to certify them: it is half a millisecond in every case (using a singale CPU core), as it depents only on the number of output of the DNN, which is 10 for all 6 DNN considered. It means 2000 image can be treated by second by each CPU core, which can be deemed {\em real-time}. We set a treshold of $70\%$ to evaluate how large the $L_1$  perturbation can be while still certifying at least this threshold of image: a threshold much lower than $70\%$ would 
mean labelling too many incoming images as uncertified, which would degrade the system too much.
...
for all 3 PCA-DNN label  for a $L1$-perturbation of $1$, and even go up to perturbation $2$ for PCA-CIFAR10-Conv and even $4$ for PCA-FMNIST-Conv.
If we are using the non PCA version, we are unable to do it, but in the case of FMNIST-Conv, with a perturbation 4 times smaller than for its PCA-version.
PCA is extremely efficient to provide real-time certification.
