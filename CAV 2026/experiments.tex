...
\begin{table}[h!]
	...
	Benchmark & MILP model: & Vhagar &  ITNE & {\em Diff} & {\em 2b+1} & {\em Best} \\\hline \hline
	MNIST-C1  & Bound $\downarrow$ &  20.80 & 12.32 & {11.14} & {\bf 10.95} & 10.85
    \\
	TO=1000s & $\%$ of gap $\downarrow$ & 120$\%$ & 29 $\%$ & {17} $\%$ & {\bf 15} $\%$& 14 $\%$
	\\ \hline
	TO=14400s  & Bound $\downarrow$ & 17.54 & 11.15 & {\bf 10.14} & 10.29 & 10.07
    \\
	 & $\%$ of gap $\downarrow$ &  84$\%$ & 17 $\%$ & {\bf 7} $\%$ & 8 $\%$ & 6 $\%$ 
    
    \\\hline \hline

	FMNIST-C1 & Bound $\downarrow$ & 41.52 & 26.61 & {\bf 23.27}  & 24.09 & 22.78
    \\
	TO=1000s & $\%$ of gap $\downarrow$ & 93 $\%$ & 20 $\%$ & {\bf 5} $\%$ & 10 $\%$ & 3 $\%$
    \\\hline 


	TO=14400s & Bound $\downarrow$ & 37.26 & 24.06 & {\bf 22.42 } & 23.80 & 22.34
    \\
	 & $\%$ of gap $\downarrow$ & 72 $\%$ &  9 $\%$ & {\bf 1} $\%$ & 9 $\%$ & 0.6 $\%$
    \\\hline \hline
	

	CIFAR-C1  & Bound $\downarrow$ & 36.57* & 27.15 & 25.94 & {\bf 24.52}& 23.29
    \\
	TO=1000s & $\%$ of gap $\downarrow$ & 55* $\%$ & 28 $\%$ & 24 $\%$& 
	{\bf 20} $\%$ & 15 $\%$
	\\\hline
    
	TO=14400s  & Bound $\downarrow$ & 31.53* & 22.73 & {\bf 21.77} & 21.81 & 20.38
    \\
	 & $\%$ of gap $\downarrow$ & 39* $\%$ & 12 $\%$ & {\bf 10} $\%$& {\bf 10} $\%$& 5$\%$
		\\\hline \hline
    

     \end{tabular}
	\caption{Comparison with Vhagar and ITNE of upper bound and gap to lower bound (lower is better) for $L_\infty=0.05$ perturbation, average over all bounds $\beta_{i,j}$. {\em Best} uses the best between our {\em Diff} and our {\em 2b+1} for each $i,j$. Lower gaps using {\em Best} means that there are cases where {\em Diff} is much better than {\em 2b+1}, and cases where it is the opposite. * for CIFAR-C1, Vhagar failed to compute bounds for some of the cases. For such cases, we used numbers from ITNE instead (which is overperforming Vhagar in every test), so these numbers are optimistic evaluation for Vhagar.}
    \label{table.classical}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{||c||c||c|c|c||c|}\hline\hline
		...
		red. MNIST-FCNN  & 1000s &&&&
		\\
		LB = & 14400s &&&&
		\\ \hline
		Full  MNIST-FCNN  & 1000s &&&&
		\\
		LB = & 14400s &&&&
		
		\\\hline \hline
		
		red. FMNIST-CNN1 & 1000s & 3.02 & 3.02 & 2.74 & 2.74
		\\
		LB = 0.96 & 14400s & 2.34  & 1.95 & 2.52 & 1.95
		\\\hline 
		
		
		full FMNIST-CNN1 & 1000s & 8.90 & 7.55 & 9.84 & 7.548
		\\
		LB = 4.16 & 14400s & 7.43 & 6.37 & 9.62 & 6.22
		\\\hline \hline
		
		
		red. CIFAR10-CNN1  & 1000s &&&&
		\\
		LB = & 14400s &&&&
		\\\hline
		
		full CIFAR10-CNN1  & 1000s &8.30 & 8.14 & 8.17 & 7.77
		\\
		LB = 2.12 & 14400s & 7.19 & 7.45 & 7.31 & 6.67
		\\\hline \hline

	\end{tabular}
	\caption{PCA input dimensions. We use PCA dimension = 20 for MNIST-FCNN, PCA dimension = 25 for FMNIST CNN1, and PCA dimension = 45 for CIFAR10 CNN1.}
	\label{table.classical_our_PCA}
\end{table}
...
%\end{table}
	...
	\subsection{Experimental results for robustness ($L_1$)}
