\section{Experimental Evaluation}
	...
	ITNE \cite{ITNE} was reimplemented using the same path as our code, except for the MILP variable $x_j$ using the classical MILP encoding plus the extra linear constraint Eq.~(\ref{eq.lpr}). The original Vhagar code \cite{vhagar} from \url{https://github.com/ananmkabaha/VHAGaR} was run on the same hardware.
...
Anyway, both are better for particular classes $(i,j)$, as {\em Best} is lower than both {\em Diff} and {\em 2b+1} in every test.
