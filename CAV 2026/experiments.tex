	...
	MNIST is a classification benchmark, we thus compute the different $\beta^{\varepsilon}_{i,j}$ bounds, for an $L_1$-deformation $\varepsilon \leq 1$. 
...
\begin{table}[h!]
    ...
    \\
	C-Conv1  & ITNE & 28.61 & 11.58 & 11.58 & $33\%$
	\\ 
...
\begin{table}[t!]
	...
	Benchmark & Timeout & MILP model: & Vhagar &  ITNE & {\em Diff} & {\em 2b+1} & {\em Best} \\\hline \hline
	  & 1000s & up. Bound $\overline{\alpha}$ $\downarrow$ &  20.80 & 12.32 & {\em 11.14} & {\em \bf 10.95} & {\em \bf 10.85}
    \\
	MNIST-C1 & & ($\%$ of gap $\downarrow$) & (120$\%$) & (29 $\%$) & ({\em 17} $\%$) & ({\em \bf 15} $\%$) & ({\em \bf 14} $\%$)
	\\
	 LB $\underline{\alpha}$= 9.51 & 14400s & up. Bound $\overline{\alpha}$ $\downarrow$ & 17.54 & 11.15 & {\bf \em 10.14} & {\em 10.29} & {\em \bf 10.07}
    \\
	  & & ($\%$ of gap $\downarrow$) &  (84$\%$) & (17 $\%$) & ({\bf \em 7} $\%$) & ({\em 8} $\%$) & ({\em \bf 6} $\%$) 
    \\\hline

	
	& 1000s & up. Bound $\overline{\alpha}$ $\downarrow$ & 41.52 & 26.61 & {\bf \em 23.27}  & {\em 24.09} & {\em \bf 22.78}
    \\
	FMNIST-C1 & & ($\%$ of gap $\downarrow$) & (93 $\%$) & (20 $\%$) & ({\bf \em 5} $\%$) & ({\em 10} $\%$) & ({\em \bf 3} $\%$)
    \\


	LB $\underline{\alpha}$= 22.21 & 14400s & up. Bound $\overline{\alpha}$ $\downarrow$ & 37.26 & 24.06 & {\bf \em 22.42 } & {\em  23.80} & {\em \bf 22.34}
    \\
	 & & ($\%$ of gap $\downarrow$) & (72 $\%$) &  (9 $\%$) & ({\bf \em 1} $\%$) & ({\em 9} $\%$) & ({\em \bf .6} $\%$)
    \\\hline
	

	& 1000s  & up. Bound $\overline{\alpha}$ $\downarrow$ & 36.57* & 27.15 & 
	{\em 25.94} & {\bf \em 24.52}& {\em \bf 23.29}
    \\
	CIFAR-C1 & & ($\%$ of gap $\downarrow$) & (55* $\%$) & (28 $\%$) & ({\em 24} $\%$) & ({\bf \em 20} $\%$) & ({\em \bf 15} $\%$)
	\\
    
	LB $\underline{\alpha}$= 19.04 & 14400s  & up. Bound $\overline{\alpha}$ $\downarrow$ & 31.53* & 22.73 & {\bf \em 21.77} & {\em 21.81} & {\em \bf 20.38}
    \\
	 & & ($\%$ of gap $\downarrow$) & (39* $\%$) & (12 $\%$) & ({\bf \em 10} $\%$) & ({\bf \em 10} $\%$) & ({\em \bf 5}$\%$)
		\\\hline
    
     \end{tabular}
	\caption{Comparison with Vhagar and ITNE of upper bound $\overline{\alpha}$ and (gap to lower bound $\underline{\alpha}$), lower is better, for $L_\infty=0.05$ perturbation, average over all bounds $\alpha_{i,j}$. * for CIFAR-C1, Vhagar failed to compute bounds for some of the cases. For such cases, we used numbers from ITNE instead (which is overperforming Vhagar in every test), so these numbers are optimistic evaluation for Vhagar. Our results are in {\em italic}. {\em Best} uses the best between our {\em Diff} and our {\em 2b+1} for each $i,j$. Lower gaps using {\em Best} means that there are cases where {\em Diff} is much better than {\em 2b+1}, and cases where it is the opposite.}
    \label{table.classical}
\end{table}


The gap is the average over all cases of the relative distance between the bound and the highest worst-case found for that case.
...
in Table \ref{table.L1} one particular bound $\beta^{.5}_{i,j}$, namely for $i=6,j=8$ as confusion between 6 and 8 happens.

We first set the PCA dimensions to have $\leq 1\%$ loss of accuracy using DNN-PCA vs original DNN:

\begin{table}[h!]
	\centering
	\begin{tabular}{|c|c c c c c c c|}\hline
		PCA dimension: & 15 & 20 & 25 & 30 & 35 & 40 & 45 
		
		\\\hline
		PCA-MNIST-FCNN  & 2$\%$ & {\bf 0}$\%$ & 0$\%$& 0$\%$ & 0$\%$& 0$\%$ & 0$\%$
		\\	
 		PCA-FMNIST-C1 & 5$\%$ &	3$\%$ &	{\bf 1}$\%$ & 0$\%$& 0$\%$	& 0$\%$ &	0$\%$ 
		\\
		PCA-CIFAR10-C1  & 16$\%$ &	14$\%$ &	12$\%$ & 10$\%$ &	14$\%$ &	5$\%$ &	{\bf 0}$\%$
		\\\hline
	\end{tabular}

	\caption{Loss of accuracy from using PCA according to the PCA dimension.
	{\bf Bold} corresponds to the dimension chosen, minimal ensuring loss 
	$\leq 1\%$.}
    \label{table.cert}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{|cc|c c c|c|}\hline
		Benchmark & Timeout & {\em Diff} & {\em 2b+1} & {\em 1b} & {\em Best} \\\hline \hline 
		MNIST-FCNN  & 1000s & 38.88 & 38.68 & 37.34 & {\bf 37.33}
		\\
		LB $\underline{\beta}$= & 14400s & 37.24 & 36.96 & {\bf 33.68} & 33.67
		
		\\\hline
		PCA-MNIST-FCNN  & 1000s & 2.429 & {\bf 2.426} & 2.68 & 2.41
		\\
		LB $\underline{\beta}$= & 14400s & 2.18 & {\bf 2.00} & 2.46 & 2.00
		\\ \hline \hline
		
 		FMNIST-C1 & 1000s & 8.90 & {\bf 7.55} & 9.84 & {\bf 7.55}
		\\
		LB $\underline{\beta} = 4.16$ & 14400s & 7.43 & {\bf 6.37} & 9.62 & {\bf 6.22}
		\\\hline

		PCA-FMNIST-C1 & 1000s & 3.02 & 3.02 & {\bf 2.74} & {\bf 2.74}
		\\
		LB $\underline{\beta}= 0.96$ & 14400s & 2.34  & {\bf 1.95} & 2.52 & {\bf 1.95}
		\\\hline  \hline
		
		
		
		CIFAR10-C1  & 1000s &8.30 & {\bf 8.14} & 8.17 & {\bf 7.77}
		\\
		LB $\underline{\beta}= 2.12$ & 14400s & {\bf 7.19} & 7.45 & 7.31 & {\bf 6.67}
		\\\hline
		PCA-CIFAR10-C1  & 1000s & 1.18 &&&
		\\
		LB $\underline{\beta}$= & 14400s & 1.07 &&&
		\\\hline \hline

	\end{tabular}
	\caption{Comparison of upper bound $\overline{\beta}$ (lower is better $\downarrow$) among Diff, 2b+1 and 1b, for both original as well as PCA-DNNs over MNIST, FMNIST and CIFAR dataset, averaged over all bounds $\overline{\beta}_{i,j}$. %Best report is the average of the best for every case.
	 %We use PCA dimension = 20 for MNIST-FCNN, PCA dimension = 25 for FMNIST CNN1, and PCA dimension = 45 for CIFAR10 CNN1.
	 }
	\label{table.L1}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{||c||c|c|c|c||c|}\hline\hline
		...
		\\\hline \hline
		MNIST-FCNN  & 0$\%$ & 0$\%$ & 0$\%$ & 0$\%$ & 0.5ms
		
		\\\hline
		PCA-MNIST-FCNN  & {\bf 80} $\%$ & 21 $\%$ & 1 $\%$ & 0 $\%$ & 0.5ms
		\\ \hline \hline
		
 		FMNIST-C1 & 
		{\bf 75} $\%$ & 50 $\%$ & 28$\%$ &	12$\%$  & 0.5ms
		\\\hline

		PCA-FMNIST-C1 & 
		92 $\%$ &	86 $\%$ &	79 $\%$ &	{\bf 70} $\%$ & 0.5ms
		\\\hline  \hline
		
		CIFAR10-C1  & 39 $\%$ & 0$\%$ & 0$\%$ & 0$\%$ & 0.5ms
		\\\hline
		PCA-CIFAR10-C1  & 88 $\%$ & {\bf 69} $\%$& 59 $\%$ & 52 $\%$ & 0.5ms
		\\\hline \hline
	\end{tabular}

	\caption{Percentage of images certified robust in real-time 
	using the Best computed $(\beta^{1}_{i,j})_{i < j \leq 10}$ 
	by {\em Diff} and {\em 2b+1}, for different values of $L_1$-perturbations. Latency overhead are the same as the number of classes is the same (=10) for all datasets. 2000 images/s can be treated per cpu core.}
    \label{table.cert}
\end{table}
