...
\noindent {\bf Discussion:} This test is relatively easy, where the best upper bound $\bar{\alpha}$ found is close to the lower bound $\underline{\alpha}$ found, with at most a $6\%$ gap. For that reason, using the 1$b$ model is not useful, as its abstraction will loose more than $6\%$. That is why it is not reported in Table \ref{table.classical}. Vhagar computes upper bounds $\bar{\alpha}$ quite far from the lower bound, with gap at least $40\%$, up to $120\%$. Although we are using Vhagar CNN and benchmark, Vhagar methodology is not well-suited for $L_\infty$, and it makes much more sense on patch explored in \cite{vhagar}. ITNE is better, with gaps from $9\%$ to $17\%$.
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c c c c c c c|}\hline
		...
		\\\hline
		PCA-MNIST-FC  & 2$\%$ & {\bf 0}$\%$ & 0$\%$& 0$\%$ & 0$\%$& 0$\%$ & 0$\%$
		\\	
 		PCA-FMNIST-Conv & 5$\%$ &	3$\%$ &	{\bf 1}$\%$ & 0$\%$& 0$\%$	& 0$\%$ &	0$\%$ 
		\\
		PCA-CIFAR10-Conv  & 16$\%$ &	14$\%$ &	12$\%$ & 10$\%$ &	14$\%$ &	5$\%$ &	{\bf 0}$\%$
		\\\hline
	\end{tabular}
...
\begin{table}[h!]
	...
	\begin{tabular}{||c||c|c|c|c||c|}\hline\hline
		...
		\\\hline
		PCA-MNIST-FC  & {\bf 80} $\%$ & 21 $\%$ & 1 $\%$ & 0 $\%$ & 0.5ms
		\\ \hline \hline
		...
		\\\hline
		PCA-CIFAR10-Conv  & 92 $\%$ & {\bf 72} $\%$& 61 $\%$ & 51 $\%$ & 0.5ms
		\\\hline \hline
	\end{tabular}
