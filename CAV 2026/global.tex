...
Different bounds were proposed in \cite{vhagar}, for an evaluation purpose (how narrow is the gap between the upper and lower bound) rather than to verify images in real-time. Namely, Vhagar defines $\alpha^{\text{perturb}}_{C,D}$, computing the perturbation necessary to switch an image classified as $C$ into an image classified as $D$:
\begin{align*}
