...
It remains to encoded the (non linear) ReLU functions of DNNs.
...
\subsection{The classical MILP encoding of ReLUs for global robustness}
	
    The classical encoding is used (with variants) by \cite{vhagar,ITNE}. It considers variables $x_j$ 
	%,\hat{x}_j$ 
	and 
	$x'_j$,
	%,\hat{x}'_j$, 
	encoding the value of neuron $j$ obtained from the image $I$ and from its perturbation $I'$, respectively. Further, for each neuron $j$, $\hat{x}_j$ and $\hat{x}'_j$ denote  the variables for $\ReLU(x_j)$ and $\ReLU(x'_j)$,
	with constraints from the classical encoding (\ref{eq11}) \cite{MILP}
	{\em independently} for $\hat{x}_j$ {\em and} for $\hat{x}'_j$ .
