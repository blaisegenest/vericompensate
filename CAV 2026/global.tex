	...
	\subsection{Optimization objective for real-time verification}

As explained in the introduction, the objective we want to maximize is 
$\beta^{\text{perturb}}_{C,D} = x_C - x_D +x'_D - x'_C$, 
for $x_C,x_D$ the value associated with the output neurons $C,D$ from input image $I$, and $x'_C,x'_D$ from perturbation $I'$. These bounds 
$\beta^{\text{perturb}}_{C,D}$ will allow us to verify in real-time 
whether a new incoming image can be certified to be robust for the perturbation.

Different bounds were proposed in \cite{vhagar}, for an evaluation purpose rather than to verify images in real-time. 

TODO: DEF OF BOUNDS IN VHAGAR. CALL BOUNDS $\alpha_C,D$, to compute the perturbation necessary to switch an image classified as $C$ into an image classified as $D$.
Notice that $\alpha_C,D$ is not symetrical, while 
$\beta^{\text{perturb}}_{C,D}$ is.

	
	\subsection{Extending the classical encoding}
	
    To encode global robustness in MILP, the classical way, used (with variants) by \cite{vhagar,ITNE}, is to consider variables $x_j,\hat{x}_j$ and 
	$x'_j,\hat{x}'_j$, encoding the value of neuron $j$ obtained from the image $I$ and from its perturbation $I'$, respectively. Further:
	\begin{enumerate}
		\item For each input neuron $j,j$ of $I,I'$, there is constraint
        $|x_j-x'_j| \leq \varepsilon$ to encode the $L_\infty$-perturbation 
		(see later how we encode $L_1$ perturbations).
		\item For each neuron $j$, we have the Re ReLU constraints 
		from the classical encoding (\ref{eq11}) \cite{MILP}
		for both $\hat{x}_j$ and $\hat{x'}_j$ independently.
	\end{enumerate}
