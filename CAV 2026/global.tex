...
implies that $\sum_{j\leq n} |x_j - x'_j| \leq c$.

Conversely, for every instance that satisfies $\sum_{j\leq n} |x_j - x'_j| \leq c$, then there is also an instance which satisfies constraints (\ref{L1constraint}),  by defining $A_j = |x_j - x'_j|$ for all $j \leq n$.

This shows that (\ref{L1constraint}) is equivalent with 
$||\vec{x} - \vec{x}'||_{L_1} = \sum_{j\leq n} |x_j-x'_j| \leq c$.
\qed
\end{proof}
...
Notice that in each case, equations not mentionned are moot.
	\qed
\end{proof}
...
Intuitively, the {\em diff values} will be small as they will be carrying the difference between both the image $I$ and its perturbation $I'$.
	...
	The decoupling between $a'$ and $a_j$ removes complicated constraints in the MILP encoding, which helps the solver. This however also means that  the "2b+1" model is not as accurate as the {\em Diff} model asymptotically.
	...
	\hspace*{10ex}\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
\caption{Possible values of $\hat{y}_i$ depending on $y_i$ for the "1b" model.}
	\label{fig.1v}
\end{figure}
...
Notice that  
$\alpha^{\text{perturb}}_{C,D}$ is not symmetrical, while 
...
\begin{enumerate}  
	\item[0] Run inference on the DNN to obtain the values $o_D$ of output neuron for each class D. Let $C$ be the class with maximal $o_{C}$. This is the predicted class from the DNN. This is the basic operation commonly done for DNN classification. % without any change.
...
\begin{proposition}
  ...
  then for all perturbed input $I'$ with  $|I - I'| \leq \text{perturb}$,
  the DNN has the same decision on $I$ and on $I'$, i.e. the DNN is robust around $I$ for perturbation $\text{perturb}$.
\end{proposition}
...
The overhead (1,2 above) afterf 0 amounts to \#Class$-1$ ADD(+) and COMP(<), then one OR($\bigvee$) of \#Class$-1$ bits, that is 19 operations in total for 10 classes. 
