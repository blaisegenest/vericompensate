...
\subsection{The classical encoding of ReLUs for global robustness}
	...
	To improve this, ITNE adds some linear constraints to the model (Eq. (3) in \cite{ITNE}), while Vhagar reduces the number of binary variables depending on the perturbation used \cite{vhagar}.
...
\begin{proposition}
    \label{Prop2}
Assuming that $y_j \in [-\gamma_j, \gamma_j]$,
and that $x'_j \in [\alpha_j,\beta_j]$,
we have that $\hat{y}_j = \ReLU(x_j=y_j + x'_j) - \ReLU(x'_j)$ is the solution of:
	\begin{align*}
		& \begin{aligned}
