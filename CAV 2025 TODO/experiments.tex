...
%
We implemented \toolname\ in Python 3.8, and Gurobi 9.52 was used for solving LP and MILP problems. We conducted our evaluation on an AMD Threadripper 7970X  ($32$ cores$@4.0$GHz) with 256 GB of main memory and 2 NVIDIA RTX 4090. The  objectives of our evaluation was to answer the following  questions:
...
Each individual node of the last layer display a similar pattern, and the pattern is similar for different images (not displayed).

We also experimented on fixing an ordering on variable in Gurobi, with a fixed selection of nodes by SAS. Indeed, Gurobi relies on a BaB procedure to compute bounds. The standard Gurobi procedure is adaptative in each branch. We compared vs the fix order provided by SAS, as well as the fixed order provided by FSB, for both accuracy and runtime (lower is better).

We obtained the following:
Gurobi order:
30nodes : 1, 3.987387317631171, 260.4883162975311, 
45nodes : 1, 4.015747873604205, 265.53007888793945, 
60nodes : 1, 4.053423060042739, 262.1407461166382, 

FSB order:
30nodes : 1, 3.9879578118285317, 246.69000577926636, 
45nodes : 1, 4.018561938508719, 247.65485453605652
60nodes : 1, 4.032634363809192, 247.81136560440063,

SAS order:
30nodes : 1, 3.9933637922653995, 264.4269104003906, 
45nodes : 1, 4.0307741092892035, 266.20811343193054, 
60nodes : 1, 4.060220618788057, 262.93414759635925, 

Here,  SAS is as efficient as Gurobi, although the ordering is fixed, meaning that the ordering compensates for the non adaptivity. However, the fixed GS ordering (with the SAS selection) is even better, with slightly better accuracy and lower runtime. As explained earlier, this is due to the fact that global ordering is preferable when branching on choices far from the optimal solution.
