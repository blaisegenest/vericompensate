...
The test are done on CIFAR image 85 and CNN-B-Adv.
...
However, the GS ordering (with the SAS selection) is better as it is general and not local to a solution, with better runtime than Gurobi, despite its staticness whereas Gurobi order is adaptative. Using SAS order for the solution branch, and GS ordering for other branches should be more efficient, but we cannot experiment that within the closed-source Gurobi solver.
