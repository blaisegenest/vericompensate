...
\begin{proof}[Proof] 
	Let $I,I'$ be pair maximizing $\beta^\varepsilon_{C,D}(\text{LAE-DNN})$.
	Let $J$ be the projected input 
...
What we need to filter out are OOD inputs leading to outputs of the DNN that are far away from ID outputs, which are the one creating large bounds $\bar{\beta}^{\varepsilon}_{C,D}$.

Reciprocally, LAE-DNN does not overcome the issue of OOD inputs: the DNN could
still provide label (with high confidence)  which does not correspond with the input. LAE-DNN only focuses on ensuring that the answer given will be robust, i.e. small changes in the input will not modify the output too much. The bottom line is that OOD is orthogonal with the focus of the paper.
...
\begin{figure*}[t!]
	...
	\caption{Training and bound computation on the pipe use case. 
		Two LAEs are used, for the input (deformation) and for the output space (plastic strain). The surrogate is learned from reduced deformation input to reduced strain output.}
	\vspace{-0.2cm}
	\label{fig.PIPE}
\end{figure*}	
