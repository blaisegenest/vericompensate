...
\begin{enumerate}
	...
	\item For vanilla DNNs, global bounds are particularly pessimistic, because inputs far away from the training dataset, i.e. not {\em In Distribution (ID)}, need to be accounted for. DNNs typically 
	behave erratically on such inputs \cite{OOD}, 
...
%, meaning that the ID directions selected are sufficient for the DNN task.
 We experimented with Linear AE (LAE): the encoding may not be as efficient as non-Linear AE, but obtaining the bound is computationally lighter. This results in latent spaces $>30$x smaller than full spaces over 3 datasets, and LAE-DNNs bounds $\bar{\beta}^\varepsilon_{C,D}$ computed are $3-15$x smaller than for DNNs.
