...
\begin{enumerate}
	...
	\item {\em Auto-Encoders for ID Projection.} For vanilla DNNs, global bounds are particularly pessimistic (see Section \ref{sec.exp}), because inputs far away from the training dataset
...
for vanilla DNNs. While explicitly {\em characterizing} ID inputs to restrict the analysis to them is impractical, we propose an efficient alternative to project inputs to an ID subspace: Learn from ID samples an autoencoder (AE), and then use the AE-DNN architecture (Fig.~\ref{fig:AE-DNN} in Section \ref{sec.pca}) to project the input on the small latent space of size $n$,%, which aligns well with ID samples.
