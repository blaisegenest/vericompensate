...
\begin{enumerate}
	...
	\item {\color{red} For vanilla DNNs, global bounds are particularly pessimistic, because inputs far away from the training dataset, i.e. not {\em In Distribution (ID)}, need to be accounted for. DNNs typically behave heratically on such inputs \cite{OOD}, 
...
learn from ID samples an autoencoder (AE), and then use the AE-DNN architecture (Fig.~\ref{fig:AE-DNN}) to first project the input on the small latent space.%, which aligns well with ID samples.
...
computes the {\em diff variables} as the difference between 2 larger values, which is {\em asymptotically} exact but results in poor {\em practical} accuracy \cite{diff}: the {\em Diff} model is more accurate in practice, with the same number of variables.
