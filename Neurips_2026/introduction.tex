...
\begin{enumerate}
	...
	\item For vanilla DNNs, global bounds are particularly pessimistic, because inputs far away from the training dataset, i.e. not {\em In Distribution (ID)}, need to be accounted for. DNNs typically behave heratically on such inputs \cite{OOD}, 
...
learn from ID samples an autoencoder (AE), and then use the AE-DNN architecture (Fig.~\ref{fig:AE-DNN} in Section 3) to first project the input on the small latent space.%, which aligns well with ID samples.
