...
In this paper, we consider the {\em global} verification problem, where we optimize over all images $I$ and perturbations $I'$ of $I$ with $|I-I'| \leq \varepsilon$. We will consider:
	...
	resp. $\hat{x}'_j=\ReLU(x'_j)$, for all neurons $j$.
	...
	\begin{itemize}
	 ...
	 $x'_j = [\boldsymbol{W}^i\cdot \hat{\boldsymbol{z}}'^{i-1}]_j+ \vb^i_j$,
	   ...
	   $\hat{x}_j$, resp $\hat{x}'_j$, uses eq. (1) with integer variable $a_j$, resp. $a'_j$.
	\end{itemize}

\label{s.L1}

	To encode an $L_1$-perturbation, we propose the following encoding {\em without} extra binary variables. Suprisingly, we could not find any reference to such an encoding in the literature.
...
\begin{proposition}
	\label{prop.L1}
 Constraints 1 and 2 above are equivalent with $\sum_{j\leq k} |x_j-x'_j| \leq \varepsilon$.
\end{proposition}
