...
\label{s.notation}
    ...
    \begin{itemize}
    \item $x_j,\hat{x}_j$, for neurons $j$ with input image $I$, 
    \item $x'_j,\hat{x}'_j$, for neurons $j$ with input the perturbed $I'$, 
    \item  $y_j = x_j - x'_j$ the {\em diff variable}, with 
    $\hat{y}_j = \hat{x}_j - \hat{x}'_j$ (and {\em not}
    $\hat{y}_j 	 = \ReLU(x_j-x'_j)$), 
	as first proposed by \citet{diff}.
...
\newpage
	...
	Mixed Integer Linear Programming (MILP) can encode ReLU DNNs faithfully:
