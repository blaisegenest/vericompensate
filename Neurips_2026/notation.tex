...
\label{s.notation}
	...
	capitalized bold $\boldsymbol{W}$ for matrices, similar to notations in \citet{crown}.
	...
	\textbf{Mixed Integer Linear Programming (MILP)} can encode ReLU DNNs faithfully:
	For an {\em unstable} neuron $n$, that is, with values 
    $x \in [\LB(n),\UB(n)]$ with $\LB(n)<0<\UB(n)$, the value $\hat{x}$ of $\ReLU(x)$ can be encoded exactly in an MILP formula with one binary/integer variable $a$ valued in $\{0,1\}$, using constants $\UB(n),\LB(n)$ with 4 constraints \citet{MILP}:
...
\end{proposition}
	...
	%For all $x \in [\LB(n),\UB(n)] \setminus 0$, there exists a unique solution $(a,\hat{x})$ that meets these constraints, with $\hat{x}=\ReLU(x)$ \citet{MILP}. The value of $a$ is 0 if $x < 0$, and 1 if $x>0$, and can be either if $x=0$. This encoding approach can be applied to every (unstable) ReLU node, and optimizing its value can help get more accurate bounds. However, for networks with hundreds of {\em unstable} nodes, the resulting MILP formulation will contain numerous integer variables, and generally bounds obtained will not be accurate, even using powerful commercial solvers such as Gurobi.
	...
	\paragraph{MILP constaints for global robustness:}
...
In this paper, we consider the {\em global} verification problem, where we optimize over all images $I$ and perturbations $I'$ of $I$ with $|I-I'| \leq \varepsilon$. We will consider:
	...
	the {\em diff variable} proposed by \citet{diff}. 
	%({\em not} $\hat{y}_j 	 = \ReLU(x_j-x'_j)$).
	%, 	similar to the Interleaving Twin-Network Encoding (ITNE) model of \citet{lipshitz,ITNE}. 
    \end{itemize}

	The classical encoding \citet{classical} uses linear variables $x_j,\hat{x}_j, x'_j,\hat{x}'_j$, as well as integer variables $a_j$,
	resp. $a'_j$ for the encoding of $\hat{x}_j=\ReLU(x_j)$,
	resp. $\hat{x}'_j=\ReLU(x'_j)$, for all neurons $j$.
	Here are the {\bf set of constraints} $\mathcal{M}^{classical}$ to encode the global robustness problem with $L_\infty \leq \varepsilon$ norm:

	\begin{itemize}
	   \item for all {\em input} neuron $j$ (in layer 0), $-\varepsilon \leq x_j-x'_j \leq \varepsilon$ [$L_\infty$ norm]
...
The objective function of the MILP program is defined in the next section.