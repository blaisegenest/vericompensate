...
faithfully	(extentions to other activation functions e.g. maxpool in \cite{DivideAndSlide}):
...
\ref{app.L1}. 
It is crucial that no exra binary variables are used, as the complexity of solving an MILP problem is bounded by $2^{bin} \times LP$ \cite{DivideAndSlide}, where $bin$ is the number of binary variables and $LP$ is the P-time complexity of solving the LP problem. The objective function of the MILP program is defined in the next section.