...
$x_C - x'_C + x'_D - x_D$, see Fig.~\ref{fig3} (left).
...
Call $dim$ the dimension of the full space, and $n$ the size of the latent space of the LAE trained on an ID dataset. LAE-Decoding is a linear function. By optimality, it can be assumed to be of rank $n$. That is, it defines $n$ linearly independent vetors of dimension $1 \times dim$.
...
That is, on ID inputs, LAE-DNN will behave largely as the original DNN. However, global bounds on the LAE-DNNs will be smaller than on the original DNN, because of the projection to the ID latent space.
Indeed, largest $x_C - x'_C + x'_D - x_D$ found 
on LEA-DNN is 6 times smaller than on DNN, and corresponds to more ID images, see Fig.~\ref{fig3} (right).
