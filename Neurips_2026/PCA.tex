...
Global upper bounds $\bar{\beta}$ obtained across the entire input space tend to be overly pessimistic on vanilla DNN, since they must accommodate all possible inputs, including inputs not In (Training) Distribution( ID) \citet{OOD}. Worst feasible solutions are indeed produced by such samples, see Fig.~\ref{fig3} (left).
Considering all inputs with the original DNN will thus produce 
high global upper bounds $\bar{\beta}$, no matter the method employed. Restricting to ID inputs seems unpractical, as there is no established characterization of ID inputs. We propose an efficient alternative.
...
maximizing $\underline{\beta}^{.5}_{6,8}=.084$ for LEA-DNN.}
\label{fig3}
\end{figure*}	
