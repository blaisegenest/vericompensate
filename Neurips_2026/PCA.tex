...
Considering the full input space on the original DNN will thus produce high global upper bounds $\bar{\beta}$, no matter the method employed to compute the bound (e.g. MILP). Restricting to ID inputs seems unpractical, as there is no established characterization of ID inputs. We propose an efficient alternative:
...
Extract the most important components of the ID dataset using an Auto-Encoder (AE), and filter out the other components from an input before feeding it to the DNN. In practice, we train the AE on the ID dataset to obtain the ID latentspace. We then define the {\em AE-DNN architecture} (Fig.~\ref{fig:AE-DNN}): first, use AE-encode to reduce to the latent space, then use AE-decode 
produce the {\em projected input} (in full space), 
and finally calling DNN on this projected input. 
...
An optimal choice of LAE-encode is the projection of input to these $n$ vectors. That is, calling LAE-decode and LAE-encode filters out features not on the ID latent-space.
...
{\em Offline:} Global bounds $\bar{\beta}$ are computed on LAE-DNN (on the full input space). The upper bound computed are better (by a factor 3 to 15) for LAE-DNN than for DNN (Table \ref{table.L1} in Section \ref{s62}).


{\em Online:} on any input, LAE-DNN performs inference, 
%, as described in the previous section. 
and the global bounds of LAE-DNN are used, certifying in real-time robustnesss for $\approx 4$ times larger $\varepsilon$ than for DNN (Table \ref{table.cert} in Section \ref{sec:realtime}).
