...
\begin{table}[b!]
    ...
    \\
     ...
     \end{tabular}
	\caption{Upper bound $\bar{\alpha}$ and (gap to lower bound $\underline{\alpha}$), lower is better, for $L_\infty=0.05$ perturbation, averaging bounds $\bar{\alpha}_{C,D}$ over all pair of classes $(C,D)$. {\em Best} computes the average $\bar{\alpha}$ using the lowest $\bar{\alpha}_{C,D}$ computed by {\em Diff} and {\em 2b+1} for each $(C,D)$. Lower gaps using {\em Best} means that there are cases $(C,D)$ where {\em Diff} is much better than {\em 2b+1}, and cases where it is the opposite. * means that VHAGaR failed to compute bounds for some cases $(C,D)$ of CIFAR-Conv. 
...
\subsection{Comparison for global robustness with VHAGaR and ITNE ($L_\infty$)}
...
\noindent {\bf Discussion:} This test is relatively easy, where the best upper bound $\bar{\alpha}$ found is close to the lower bound $\underline{\alpha}$ found, with at most a $6\%$ gap. Using the {\em 1b} model is not useful, as its abstraction will lose more than $6\%$. 
...
\subsection{Experimental results for $L_1=1$ and LAE-DNN.}
...
We recall that LAE-DNNs are generated following the process described in Section 5: for each latent space dimension, we report in Table \ref{table.LAE} the accuracy loss from using LAE-DNN rather than the original DNN.
...
\begin{table}[b!]
	...
	\caption{Loss of accuracy of LAE-DNN vs DNN (no LAE) according to the latent space dimension.
...
\begin{table}[b!]
	...
	between ITNE and {\em Best} of {\em Diff, 2b+1 and 1b}, for original DNNs as well as LAE-DNNs over MNIST, FMNIST and CIFAR datasets. %Best report is the average of the best for every case.
...
Comparing with and without LAE, as expected, LAE-DNNs allow reaching much smaller upper bounds $\bar{\beta}$ than without LAE, $15$x for MNIST-FC, $3$x for FMNIST-Conv, and $7$x for CIFAR10-Conv. MNIST-FC is harder to solve than FMNIST-Conv, although they are comparable benchmarks, because fully connected architectures are richer than convolutional architectures of the same size. LAE-CIFAR10-Conv is the hardest test, because the dimension (60) is not as reduced as for MNIST and FMNIST (20 and 25).
...
\subsection{Real-Time verification of Robustness}
