...
\begin{table}[b!]
    ...
    \\
     ...
     \end{tabular}
	\caption{Upper bound $\bar{\alpha}$ and (gap to lower bound $\underline{\alpha}$), lower is better, for $L_\infty=0.05$ perturbation, averaging bounds $\bar{\alpha}_{C,D}$ over all pair of classes $(C,D)$. {\em Best} reports the average $\bar{\alpha}$ using the lowest $\bar{\alpha}_{C,D}$ computed 
...
\label{sec.exp}
	...
	ITNE \citet{ITNE} was reimplemented using the same path as our code, except for the MILP variable $x_j$ using the classical MILP encoding plus the extra linear constraint Eq.~(\ref{eq.lpr}). The Julia VHAGaR code 
	...
	from \citet{vhagar} ("conv1"). We will also consider 3 LAE-DNNs, see Section~\ref{s62}.
...
VHAGaR computes upper bounds $\bar{\alpha}$ quite far from the lower bound, with a gap $40-120\%$. Although we are using its benchmarks, VHAGaR methodology is not well-suited for $L_\infty$ (it is more efficient on {\em Patch} \citet{vhagar}): ITNE is better, with gaps from $9-17\%$.
...
Recall that LAE-DNNs are generated following the process described in Section 5: for each latent space dimension, we report in Table \ref{table.LAE} the accuracy loss from using LAE-DNN vs using the original DNN. {\color{red} Concerning the LAE algorithm, we experimented with PCA 
\citet{Paco}}.
We set the dimension at the minimal dimension that results in at most 1$\%$ 
loss of accuracy, that is $20, 25$ and $60$ for  MNIST-FC, FMNIST-Conv and CIFAR10-Conv respectively, i.e. $>35$x dimension reduction.
...
$L_1$ perturbations, from 1.5 to 600. Using 
...
would reach for CIFAR10-Conv bound less than three times as high as the bound of our MILP models for LAE-CIFAR10-Conv. For FMNIST-Conv, it is 2 times.
...
\begin{table}[t!]
	...
	\begin{tabular}{|c|cccc|c|}\hline
		...
		LAE-FMNIST-Conv & 
		92$\%$ &	86$\%$ &	79$\%$ & {\color{blue} 70$\%$}  & 0.5 ms
		\\\hline
		
		CIFAR10-Conv  & 39$\%$ & 0$\%$ & 0$\%$ & 0$\%$ & 0.5 ms
		\\ 
		LAE-CIFAR10-Conv  & 92$\%$ & {\color{blue} 72$\%$} & 61$\%$ & 51$\%$ & 0.5 ms
		\\\hline
	\end{tabular}
	...
	{\color{blue} Blue} depicts the largest perturbation ensuring $\geq 70\%$ of images being robust. The latency overhead is the same as the number of classes (10) for all datasets.}
