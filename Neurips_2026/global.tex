...
$,
where $x_{C,D},x'_{C,D}$ are the output values of class $C,D$ from image $I$ to the perturbed image $I'$ or the differential between $I,I'$ output values, respectively.
...
The overhead (steps 1,2 above) on top of the standard inference step 0, 
amounts to $2k-1$ operations, where $k$ is the number of classes. 
That is, 19 operations in total for $k=10$ classes (cases of MNIST, Fashion MNSIT and CIFAR-10). More precisely: $(k-1)$ ADD(+), 
$(k-1)$ COMP(<) and $1$ OR($\bigvee$) of ($k-1$) bits.
This can be performed extremely fast, even on limited hardware.