\section{Diff MILP Models for Global Robustness}

\paragraph{Encoding $L_1$-perturbations:}
\label{s.L1}
...
Let $n$ be the number of input neurons. 
To encode $L_1$-perturbations, 
we use $n$ additional variables $(A_j)_{j \leq n}$ that are linear only. We have $\sum_{j\leq n} |x_j-x'_j| \leq \varepsilon$ iff
...
\subsection{The exact {\em Diff} MILP encoding for global robustness}
	\label{s.diff}
...
We now present our main methodological contribution, the {\em Diff} MILP encoding. It considers variables $x'_j,\hat{x}'_j, y_j, \hat{y}_j$, where $y_j, \hat{y}_j$ are the {\em Diff variables} \citet{diff}, with 
	...
	Note that, since the image $I$ and the deformation $I'$ have perfectly symmetrical roles and assuming that $\gamma_j$ is an {\em upper} bound of $y_j$, then $-\gamma_j$ is a {\em lower} bound for $y_j$.
...
We first display on Figure \ref{fig.2v} the function $f(x'_j,y_j)=\hat{y}_j$ for $y_j \in [- \gamma_j, \gamma_j]$. 
    ...
    Between these two linear planes, we have two more presenting intermediate cases, so 4 linear planes in total.
...
To encode both $\hat{y}_j$ and $\hat{x}'_j$, the {\em Diff} encoding needs 2 binary variables, similar to the classical encoding.
...
The LP relaxation of ${\cal M}^{classical}$ will miss this totally, while the LP relaxation of the Diff model contains such important bounds, including: 
%(see section \ref{s.1b}) and Fig.~\ref{fig.1v}):
\begin{align}
	\label{eq.lpr}
\frac{y_j-\gamma_j}{2} \leq \hat{y}_j \leq \frac{y_j+\gamma_j}{2}
\end{align}
%which is not the case of ${\cal M}^{classical}$. 
This is a simplification (due to symmetry) of the additional linear constraint (3) of \citet{ITNE}.
% used explicitly in ITNE.
...
Intuitively, the {\em diff values} will be small as they will be carrying the difference between both the image $I$ and its perturbation $I'$.
    ...
    \subsection{Abstraction of {\em Diff}: the {\em 2b+1} and {\em 1b} models}

\label{s.1b}


	We propose an abstraction of the {\em Diff} encoding: 
	the binary variables $a'$ appearing in 
    Prop. \ref{Prop2} can be decoupled from the binary variable $a'_j$ 	appearing in the classical encoding of $\hat{x}'_j = \ReLU(x'_j)$. 
	...
	both variables $ a$ and $a'$ used in the encoding of $\hat{y}_j$ are kept binary. We call this the {\em 2b+1} model.
	%, which uses 2 binary variables per neuron, as the classical and {\em Diff} encodings.
	The decoupling between $a'$ and $a_j$ removes constraints in the MILP encoding, which helps the solver. However, the {\em 2b+1} model is not as accurate as the {\em Diff} model asymptotically.
...
\fi
    ...
    we introduce the efficient {\em 1b} model which only uses the diff variables $y_j,\hat{y}_j$, and do not consider variables $x'_j,\hat{x}'_j$. 
	% as depicted in Fig.~\ref{fig.1v}. This is a projection of Fig.~\ref{fig.2v} onto $y_j,\hat{y}_j$.
    

    \begin{proposition}
    \label{prop3}
    Given $y_j \in [-\gamma_j,\gamma_j]$, 
the range of values $\hat{y}_j$ can take (see Figure in Supp. Material) corresponds to solutions of the following {\em 1b} MILP program with binary variable $a \in \{0,1\}$:
	\begin{align*}
