...
% or the differential between $I,I'$ output values, respectively.
...
Let $N$ be the number of neurons of a DNN. Compared with local robustness which considers only variables $(x'_j)_{j \leq N}$ for the perturbed image $I'$, global robustness considers also variables $(x_j)_{j \leq N}$ for the ({\em non}-fixed) input image $I$, and thus necessitates double the number of (binary) variables (for $I$ and for $I'$). Worst-case complexity being exponential in the number of binary variables, global robustness (e.g., ITNE and VHAGaR \citet{ITNE,vhagar}) is much harder than local robustness.

\subsection{Encoding $L_1$-perturbations for input variables}
\label{s.L1}

The standard encoding of $L_\infty$-perturbations bounded by $\varepsilon$ uses
linear constraint $x_j-\varepsilon \leq x'_j \leq x_j+\varepsilon$ 
for each {\em input} neuron $j$. 

\smallskip

We now explain how to encode $L_1$-perturbations 
$\sum_{j\leq n} |x_j-x'_j| \leq \varepsilon$, using additional variables that are linear only. Let $n$ be the number of input neurons. 
We use $n$ additional linear variables $(A_j)_{j \leq n}$, with the following  constraints:
...
for a neuron $j$ of layer $\ell$, the value of variable $x_j$ is defined by the constraint $x_j = \sum_{i \in \ell-1} \alpha_i \hat{x}_i$, which is a simple linear operation from the output $(\hat{x}_i)_{i \in \ell-1}$ of neurons $i$ in layer $\ell-1$.
...
if the {\em Diff} variables $y_j = x_j - x'_j$ are considered as defined in Section 2, with $\hat{y}_j = \hat{x}_j - \hat{x}'_j$, then we simply have 
...
\subsection{The classical encoding of ReLUs for global robustness}
    ...
    The classical encoding is used (with variants) by VHAGaR \citet{vhagar} and ITNE \citet{ITNE}. For each neuron $j$, variables $x_j,\hat{x}_j$ and $x'_j,\hat{x}'_j$ are considered. For each ReLU $j$, a binary variable $a_j$ and related constraints from the classical encoding (\ref{eq11}) \citet{MILP} are used to set $\hat{x}_j$, and an {\em independent} binary variable $a'_j$ and related constraints from the classical encoding are used to set $\hat{x}'_j$. We call this encoding $\mathcal{M}^{classical}$.
	
	The key values to compute are the {\em diff}erential values of $y_j$ between $x_j$ and $x'_j$. Most of the values of $y_j$ are likely small because they are inherited from a small perturbation between $I$ and $I'$.
	However, $\mathcal{M}^{classical}$ only maintains $x_j$ and $x'_j$ separately, and both can take large values. This results into poor evaluation of $y_j$. This problem was first raised and experimentally demonstrated in \citet{diff}.

	Similarly, the standard way to make the MILP model more efficient is to linearly relax some of the binary variables (see Section~\ref{s.notation}). The problem is that the LP relaxation of $\mathcal{M}^{classical}$ is extremely inaccurate, as variables $x_j,x'_j$ dependencies are due solely on the function definition through common ancestor variables in previous layers - as this is approximated with LP, most of the dependencies will be lost. On top of being an issue with partial MILP models, LP relaxation is also used during the Branch and Bound process internal to MILP solvers. 
	%to provide bounds for each branch. 
	So even without explicit LP relaxation, the classical MILP encoding of \citet{MILP}, while asymptotically accurate, will result in looser bounds in practice. 
	
	To improve this, ITNE adds some linear constraints to the model (Eq. (3) in \citet{ITNE}, see also Section \ref{s.diff}), while VHAGaR reduces the number of binary variables depending on the perturbation used \citet{vhagar}.
	...
	We now present our main methodological contribution, the {\em Diff} MILP encoding. It considers variables $x'_j,\hat{x}'_j, y_j, \hat{y}_j$, where $y_j, \hat{y}_j$ are the {\em Diff variables} \citet{diff}, with 
	$y_j = x_j - x'_j$ and $\hat{y}_j= \ReLU({x}_j) - \ReLU(x'_j)$. 
	A first important observation is that, since the image $I$ and the deformation $I'$ have perfectly symmetrical roles and assuming that $\gamma_j$ is an {\em upper} bound of $y_j$, then $-\gamma_j$ is a {\em lower} bound for $y_j$.
    ...
    However, unlike ITNE, 
	%Interleaving Twin-Network Encoding (ITNE) of \citet{lipshitz,ITNE} which relies on the classical MILP encoding (\ref{eq11}) \citet{MILP} to compute $\hat{y}_j$ from the difference between 
	%$\ReLU(x'_j + y_j) = \ReLU(x_j) = \hat{x}_j$
	%and $\hat{x}'_j$, 
	...
	$\ReLU(x_j)$ and using $\hat{y}_j= \ReLU(x_j) - \ReLU(x'_j)$, which is correct but does not encode explicitly that $\hat{y}_j$ is small when $y_j$ is small, since both ReLUs can be large and they are encoded independently, see also \citet{diff}. Instead, we study the function from $(x'_j,y_j)$ to $\hat{y}_j$, and encode it into the novel MILP {\em Diff} model.
    


	%	(This model has the same binary set, although the meaning of the binary variable for $y_i$ is somehow different.)
	...
	\begin{tikzpicture}[scale=0.65]
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Function $(y_j,x'_j) \mapsto	\hat{y}_j=\ReLU(x_j=y_j+x'_j)-\ReLU(x'_j)$}
	\label{fig.2v}
\end{figure}
...
Recall first that $a'_j$ is the binary variable used in the 
...
\begin{proposition}
    \label{Prop2}
Assuming $y_j \in [-\gamma_j, \gamma_j]$ and $x'_j,y_j+x'_j \in [\alpha_j,\beta_j]$ and $\alpha_j<0<\beta_j$, we have that $\hat{y}_j = \ReLU(x_j=y_j + x'_j) - \ReLU(x'_j)$ is the solution of the MILP program:
	\begin{align*}
		& \begin{aligned}
...
Equations not mentioned are moot, as shown in the appendix.
	\qed
\end{proof}
...
This is a simplification (due to the noticed symmetry) of the additional linear constraint (3) of \citet{ITNE} used explicitly in ITNE.
...
First, we know the constraints for $\hat{x}_i'=\ReLU(x_i')$ are exact (see 
Prop.~\ref{Prop1}). 
...
Intuitively, the {\em diff values} will be small as they will be carrying the difference between both the image $I$ and its perturbation $I'$.
	...
	of \citet{MILP}, while fully accurate, will likely result in looser bounds as well. Notice that this was already witnessed in \citet{ITNE}, and some additional constraints were added to make the classical model more accurate: the {\em diff variables} were used as specific constraints added to the classical encoding (Eq. (3) in \citet{ITNE}). 
	%We will compare the different methods in Table \ref{table.classical}.
	...
	The variable $a'_j$ used for $x'_j$ is thus linearly relaxed, while
	both variables $ a$ and $,a'$ used in the encoding of $\hat{y}_j$ are kept binary. We call this the {\em 2b+1} model, which uses 2 binary variables per neuron, 
	as the classical and {\em Diff} encodings.
	The decoupling between $a'$ and $a_j$ removes constraints in the MILP encoding, which helps the solver. This, however, also means that the {\em 2b+1} model is not as accurate as the {\em Diff} model asymptotically.
...
\end{figure}
    ...
    we introduce an efficient {\em 1b} model which only uses the diff variables $y_j,\hat{y}_j$. Again, we study the range of values $\hat{y}_j$ can take according to the value of $y_j$, as depicted in Fig.~\ref{fig.1v}. This is a projection of Fig.~\ref{fig.2v} onto $y_j,\hat{y}_j$.
    

    \begin{proposition}
    \label{prop3}
    Given $y_j \in [-\gamma_j,\gamma_j]$, 
    $\hat{y}_j$ is a solution of the MILP program :\begin{align*}
		\hat{y}_j &\leq a \gamma_j               &\quad \hat{y}_j &\geq y_j - a \gamma_j \\
		\hat{y}_j &\geq (a-1) \gamma_j           &\quad \hat{y}_j &\leq y_j + (1-a) \gamma_j
	...
	As the {\em 1b} model is a projection of the {\em Diff} model and also of the {\em 2b+1} models, 	the LP relaxations of the {\em Diff} and the {\em 2b+1} models contain (\ref{eq.lpr}).  Proof of Proposition~\ref{prop3} is in the appendix.
	%Notice that (\ref{eq.lpr}) corresponds to Eq. (3) in \citet{ITNE}, 
	...
	\iffalse
	Based on the above constraints, we can sketch this simplified model:
	\begin{enumerate}
		...
		\item Set linear constraints. In this case, since the meaning of $y_i$ is $x_i-x'_i$, these constraints will not use the bias.
