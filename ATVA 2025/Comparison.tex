...
%\textcolor{red}{Fig2 page10 is intended as a running example: We show how $s_{FSB}$ behaves on Fig2 (page10-11). Page12 (Fig4 and above), we explain how SAS computes the same case and compares with FSB. We will add explicit references to the case $c<0,d<0$ leading to $s_{FSB}=\frac{cd}{4}$, which corresponds in Figure4 to either case	
...
Compare $s_{FSB}(a)$ with $imp(a)$, the real improvement on the {\em maximal bound} computed by pMILP$_X(z)$ when opening the $\ReLU$ of node $a$ ($X=\{a\}$), yielding 
$\val(\hat{a}) = \ReLU(\val(a))$, versus having $X=\emptyset$, for which 
$\val(\hat{a})$ can be any value in the triangle approximation (Figure \ref{triangle})), 
according to Prop. \ref{lp}.
...
That is, opening node $a$ is not improving the bound $\UB(z)$, as correctly predicted by the score $s_{FSB}(a)= imp(a) = 0$.
